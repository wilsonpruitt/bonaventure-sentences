# Next session — **DISTINCTIO XXXII now FULLY Tier-2** (2026-05-30 — all 9 units: littera + divisio + a1-q1/q2 + a2-q1/q2 + a3-q1/q2 + dubia; commits `509ee3b`→`ffc43e5`, plus a3-q1 fix `bbe8b02`). **NEXT ACTION = dispatch `bon-sent-II-d33-littera`** (Lombard's text of DISTINCTIO XXXIII). **d.33 littera range:** raw **≈54236** (`Cap. I.` of DIST. XXXIII; the running head `DISTINCTIO XXXlil.` is OCR-mangled, sits raw ≈54296) → immediately before **`COMMENTARIUS IN DISTINCTIONEM XXXIII.`** (raw **≈54448**). Littera = Lombard chapters **Cap. I, II, III** (verify exact count at 450 dpi — IA OCR interleaves columns, do NOT trust raw line order for chapter sequence). Printed pages **≈778–781** (offset pdf=printed+22; `DIST. XXXIII. DIVISIO TEXTUS. 781` running head confirms divisio on printed p.781). **d.33 commentary structure after the littera** (mirrors d.32 — 3 art × 2 q): `COMMENTARIUS` (raw ≈54448) → `DIVISIO TEXTUS.` (raw ≈54456) → `TRACTATIO QUAESTIONUM.` (raw ≈54485) → ART I q1 (raw ≈54508) / q2 (raw ≈54670) → ART II q1 (raw ≈54863) / q2 (raw ≈55064) → ART III q1 (raw ≈55243) / q2 → DUBIA. **d.33–40 skeletons were REGENERATED 2026-05-30 with a FIXED auto-chunker** (commits `95b6a45` tooling + `d5bec59` skeletons) — the d.32-class chunker bugs are now fixed, so the old `d33-divisio-dup2`/`d33-dubia-dup2` dups are GONE and every d.33–40 distinction has a clean 1 littera + 1 divisio + 1 dubia + correct quaestio count. **Still ALWAYS run the Primo/Secundo-quaeritur alignment check at promotion** (cheap insurance), but the gross structural misses are pre-cleared. Template = `vol2/bon-sent-II-d32-littera.md` (littera) / `d32-divisio.md` (divisio). **HAND-OFF into d33-littera:** NONE (DIST. XXXIII opens fresh; d32-dubia closed cleanly at top of p.778 R-col before `Cap. I.`; the p.778 NOTAE block belongs to DIST. XXXIII). **No decade polish until d.40.**
