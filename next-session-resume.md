...
> # ★★★ THE ITINERARIUM IS COMPLETE — ALL TEN CHUNKS TIER 2, pp. 295–316.
>
...
>
> ## ▶▶▶ **NEXT = THE WORK-CLOSE POLISH GATE, AND THEN THE DEPLOY. BOTH FIRE AT p. 316 AND THEY FIRE TOGETHER** — the first time in this work that the two cadences coincide. Nothing else is owed before them, and **no new chunk should be started until the gate closes.**
>
> **The gate is four passes** (repo CLAUDE.md § "Polish-gate cadence"): **(1)** `[?]`-flag resolution over pp. 295–316 — **there are none open in any of the ten chunks, so this pass has nothing to do and should be recorded as such, not skipped silently**; **(2)** full-corpus style scan (already run every commit, CLEAN at 89 files); **(3)** the boundary-integrity sweep over every mid-page and leaf-crossing boundary in pp. 295–316 — **⚠⚠ THE LEAF CROSSINGS MUST BE COUNTED BY HAND: `seam-screen.py` is structurally blind to them** (the Breviloquium gate's finding, where 11 crossings existed only because pass 3 counted them itself); **(4)** disk cleanup — `rm -f raw/vision/vol5/*.png /tmp/colcrop/*` (the Breviloquium gate reclaimed ~3.7 GB this way).
>
> **Then the deploy** (recipe at the foot of this file — note the TWO index steps before `build-content.mjs`). ⛔ **Deploy and push are BOTH protected and BOTH need Wilson's explicit per-action OK.** `master` is now **23 commits ahead of origin.**
>
...
> ## ⚠ **Cadence — THE GATE AND THE DEPLOY ARE BOTH DUE NOW, AT p. 316.** ONE polish gate for this work (24 pp), at the work close; **deploy boundary = the same point.** The Scholion has landed, so **both fire together** — the four-pass work-close gate (flag resolution · full-corpus style · boundary sweep over every mid-page and leaf-crossing boundary in pp. 295–316 · disk cleanup) and then the deploy. **The boundary sweep must count leaf crossings BY HAND — `seam-screen.py` is structurally blind to them** (the Breviloquium gate's finding). **Do not start `de-reductione` until the gate closes.**
