...
>
> ## ✅ `prol` · `capitula` · `c1` · `c2` · `c3` · `c4` · **`c5` (2026-08-13, `da12867`)** — **pp. 295–310, zero `[?]` flags in all seven.** Remaining: **c6 · c7 · scholion.**
> ## ▶▶▶ **NEXT CHUNK = `bon-itin-c6`.** Index claims pp. 310–312; **the claim is worth nothing** — close the end positively from the in-place `Cap. VII.` heading.
>
...
>
> **Hand-off `bon-itin-c6` inherits from `bon-itin-c5` — re-derive it, do not adopt it:**
> - **p. 310 nn. 6–9 are c6's.** n. 6 (`Exod. 25, 19: Cherub unus sit in latere uno, et alter in altero…`) anchors at *ut alter Cherub iuxta alterum statuatur*⁶ in Cap. VI § 1, **right** column. Block prints **7 L / 2 R**, so **three of c6's four notes print on the LEFT; verify every column yourself.**
...
> - **Cap. VI opens p. 310 RIGHT column**, ~25 % down, at `1. Post considerationem essentialium elevandus est oculus intelligentiae ad contuitionem beatissimae Trinitatis…`, under the `Cap. VI.` heading + subtitle *De speculatione beatissimae Trinitatis in eius nomine, quod est bonum.*
> - **Marginalia on p. 310 that are c6's:** "Gradus 6." · "Duo inferuntur." · "Quid bonum et summa diffusio." · "Duplex productio." · "Ratio."
> - **p. 310 gutter = 1397.** ⚠ **Take it from the body+FOOTER profile** — Cap. VI's heading and subtitle sit inside the RIGHT column and will blow out a body-only window, exactly as p. 308's did for c4.
>
...
>
> ## ✅ Verification suite at the `bon-itin-c5` commit: `check-vol5-apparatus.py` **86 chunks / 805 entries** all passed (p. 310 correctly PENDING nn. 6–9; `KNOWN_TOTALS` fed through p. 310) · `check-vol5-census.py` rosters agree **86/86**, **seven** page-crossing runovers on the ledger · `polish-style-scan --volume 5` CLEAN (86 files) · `build-content.mjs` **2019/2019, 6 books** · `build-citations.py` clean; c5 contributed **24 records, ZERO dangling**, corpus QA flags unmoved at 226 (but see QA job 0 above — two of c5's records are wrongly-resolved anaphora that the flag count does not see).
