# Next session — Vol II **DISTINCTION 2 `d2-p1-a1-q2`** Tier-2 (session 17 — littera + divisio + a1-q1 DONE)

Updated 2026-05-16 at close of session 16 (`d2-p1-a1-q1` promoted to Tier-2; d.2 now 3/17).

Head commit: `7b65c1c` — vol2 d.2 promote bon-sent-II-d2-p1-a1-q1 to Tier-2 (session 16). Resume commit follows.
...
- **Vol II DISTINCTION 2 RE-CHUNKED (session 13, commit `ee509db`)** — 17 correct skeletons. **3/17 Tier-2**: `d2-littera` (session 14, `90ec205`) + `d2-p1-divisio` (session 15, `5b47955`) + `d2-p1-a1-q1` (session 16, `7b65c1c`). Remaining 14 skeletons; next = `d2-p1-a1-q2` (QUAESTIO II, raw 4697–4912). See "Next" below.
- Site build: 2 books, 873 chunks, **431 translated** (littera + divisio + a1-q1 counted).
...
## Next: DISTINCTION 2 — `d2-p1-a1-q2` Tier-2 (littera + divisio + a1-q1 DONE)

**`d2-littera` (s14, `90ec205`), `d2-p1-divisio` (s15, `5b47955`), `d2-p1-a1-q1` (s16, `7b65c1c`) are Tier-2.** **Session 17 = promote `bon-sent-II-d2-p1-a1-q2` to Tier-2** — QUAESTIO II, *Utrum omnium aeviternorum sit unum aevum*, raw **4697–4912** (~216 raw lines). Then a1-q3, a2-q1… at ~1/session. Pre-rechunk backup: `_backup-d2-pre-rechunk-20260515/` (gitignored).

**`d2-p1-a1-q2` starting points (mapped while reading pp.55–58 for a1-q1):**
- Opens at the OCR-garbled running head `QU.\ESTIO 11.` = **QUAESTIO II** (raw ~4698), title *Utrum omnium aeviternorum sit unum aevum* (`Utrum omnmm aeviternorum. sit wmim aevum`), body opener `Secundo quaeritur, utrum spiritualia habeant mensuram unam per essentiam…`. **Begins in normal type at the TOP of printed p.58**, above q2's own footer apparatus — i.e. p.58's main body is q2 even though q1's scholion sits in small two-column type at the foot of p.57→top of p.58. q2 then runs p.58 onward (extract p.58 — already cropped at `/tmp/colcrop/vol2-p058-*` — plus p.59+ fresh until QUAEST. III running head).
- **Cross-chunk footer (q1→q2): p.58 footer apparatus is q2's, NOT q1's.** Session 16 confirmed p.58 L-col footer notes 1–5 (`Cap. 5. § 5…`; `Aristot., I. Topic. c. 6…`; `Cod. Q interserit Angelis…`; `Plurimi codd. minus congrue enim`; `Dialog. de Veritate, c. 13…`) anchor in the q2 body. Start q2's apparatus from p.58 footer note 1 by body anchor; no backfill into committed `d2-p1-a1-q1` (its 26 entries close cleanly at p.57; its scholion I–III has no apparatus). Re-walk p.58 footer in printed order for q2.
- **Full cascade-quaestio discipline applies** (Vol II override): `extract-pages.py --volume vol2 --pages <printed> --dpi 450` then `colcrop.py vol2 <printed>`; L col then R col, body then footer; reflow column-by-column. Footnote numbering restarts per printed page; build the per-page→chunk crosswalk and watch the q2↔q3 cross-chunk footer split at the next boundary.
- Apparatus-count heuristic noise applies as in q1 (two-column cascade footers; ground-truth = printed footers walked page by page).
- Crops cached: pp. 52–58 (`/tmp/colcrop/vol2-p05{2..8}-…`). p.59+ need a fresh extract.

### d.2 chunk set (17, raw `bonaventure_vol2_raw.txt`, lines 4264–6627; d.3 starts 6628)

`d2-littera` 4264-4417 ✓ · `d2-p1-divisio` 4418-4462 ✓ · `d2-p1-a1-q1` 4463-4696 ✓ · **`d2-p1-a1-q2` 4697-4912 ← NEXT** · `d2-p1-a1-q3` 4913-5097 · `d2-p1-a2-q1` 5098-5212 · `d2-p1-a2-q2` 5213-5333 · `d2-p1-a2-q3` 5334-5449 · `d2-p1-dubia` 5450-5529 · `d2-p2-divisio` 5530-5554 · `d2-p2-a1-q1` 5555-5743 · `d2-p2-a1-q2` 5744-5886 · `d2-p2-a2-q1` 5887-6125 · `d2-p2-a2-q2` 6126-6244 · `d2-p2-a2-q3` 6245-6381 · `d2-p2-a2-q4` 6382-6500 · `d2-p2-dubia` 6501-6627.
