# Next session — **d24-p1-a2-q3 DONE 2026-05-29 → next action: dispatch `bon-sent-II-d24-p1-a2-q4` (ARTICULUS II QUAESTIO IV *Utrum intellectus agens et possibilis sint una potentia, an diversae* [= *per possibilem et agentem*]; skeleton, opens raw 39754 QUAESTIO IV on p. 567). Hand-off RECEIVED from a2-q3: NONE — q3's footers + SCHOLION I are self-contained within pp.565–567; QUAESTIO IV opens lower on p.567 (raw 39754) with its own fresh footer sequence (p.567 left-bottom footers Aristot. III de Anima text.17 / II Phys text.70 / etc. belong to q4). q4 is the LAST quaestio of Pars I ART II — after it come Pars I DUBIA (`d24-p1-dubia`, raw footers=8), then Pars II. Extract pp.567+ crops as needed (`/tmp/colcrop/vol2-p567-*` cached). Apparatus-count audit shows raw=31 footers for a2-q4 (spans ~pp.567–569, multi-page; walk the raw range, heuristic over-counts via two-column cascade).**
> **ARTICULUS II = *De divisione potentiarum animae* (4 questions, all in Pars I); TRACTATIO (raw 39105–39130, verbatim): Primo *per cognitivam et affectivam, sive per rationem et voluntatem* (= a2-q1, DONE); Secundo *per portionem superiorem et inferiorem* (= a2-q2, DONE); Tertio *per deliberativum et naturale* (= a2-q3, DONE — 12 apparatus entries, pp.565–567, SCHOLION I); Quarto *per possibilem et agentem* (= a2-q4, NEXT — OCR title *Utrum intellectus agens et possibilis sint una potentia, an diversae*). NOTE: Pars II numbering (p2-a*, p2-dubia) is auto-chunk-suspect — re-chunk-verify before promoting Pars II. Stray dups still pending cleanup: `d24-p1-dubia-dup2` (apparatus audit flags it — git rm when the real `d24-p1-dubia` chunk is promoted).**
