# Next session — **DISTINCTIO XXXIII littera now Tier-2** (2026-05-30 — `bon-sent-II-d33-littera`, Lombard's text Cap. I–V, commit `6fefab3`; range raw 54235–54444, printed pp. 778–781, 19 NOTAE footers, page-split 3+7+8+1). **NEXT ACTION = dispatch `bon-sent-II-d33-divisio`** (the COMMENTARIUS + DIVISIO TEXTUS + TRACTATIO QUAESTIONUM of DIST. XXXIII). **d.33 divisio range:** raw **54448** (`COMMENTARIUS IN DISTINCTIONEM XXXIII.`) → DIVISIO TEXTUS (raw 54456) → TRACTATIO QUAESTIONUM (raw 54485) → ends immediately before ART I q1 opener (raw ≈54508). Printed page **781** (offset pdf=printed+22; running head `DIST. XXXIII. DIVISIO TEXTUS. 781`). **d.33 commentary structure after the divisio** (mirrors d.32 — 3 art × 2 q): ART I q1 (raw ≈54508) / q2 (raw ≈54670) → ART II q1 (raw ≈54863) / q2 (raw ≈55064) → ART III q1 (raw ≈55243) / q2 → DUBIA. **HAND-OFF into d33-divisio:** the littera FORWARDED the p.781 footers BELOW its single `NOTAE AD LIBR. SENTENTIARUM` note ([^19] = `Loc. cit. parum inferius. Locus Scripturae est Gen. 9, 25.`) — i.e. the whole `NOTAE AD COMMENTARIUM.` block starting `¹ Vat. contrarietatem, cui lectioni favet textus determina…` belongs to the divisio (its footer numbering restarts at its own 1). The littera body closes at the TOP of the p.781 R-col with "…Sed in mysterio illud dictum est." (raw 54444), grammatically complete; the COMMENTARIUS opens fresh below it. NOTE: littera was Cap. **I–V** (5 chapters, not 3 — the old resume estimate of I/II/III was low; Cap. IV/V are *An illud peccatum sit primis parentibus dimissum* + *Quomodo peccata parentum visitentur in filios*). **d.33–40 skeletons were REGENERATED 2026-05-30 with a FIXED auto-chunker** (commits `95b6a45` tooling + `d5bec59` skeletons) — the d.32-class chunker bugs are now fixed, so the old `d33-divisio-dup2`/`d33-dubia-dup2` dups are GONE and every d.33–40 distinction has a clean 1 littera + 1 divisio + 1 dubia + correct quaestio count. **Still ALWAYS run the Primo/Secundo-quaeritur alignment check at promotion** (cheap insurance), but the gross structural misses are pre-cleared. Template = `vol2/bon-sent-II-d32-divisio.md` (divisio) / `d32-littera.md` (littera). **No decade polish until d.40.**
