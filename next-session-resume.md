# Next session — **DISTINCTIO XXXIII divisio now Tier-2** (2026-05-30 — `bon-sent-II-d33-divisio`, COMMENTARIUS + DIVISIO TEXTUS + TRACTATIO QUAESTIONUM, commit `45d1f34`; range raw 54448–54508, printed p.781, 2 NOTAE AD COMMENTARIUM footers; received the littera's forwarded p.781 footer block and confirmed the littera's single NOTAE AD LIBR. SENTENTIARUM note [^19] retired). **NEXT ACTION = dispatch `bon-sent-II-d33-a1-q1`** (ART I = *De originali quoad causam praecedentem*; QUAESTIO I = *Utrum peccata proximorum parentum filiis imputentur vel ad eos transmittantur*). **d.33-a1-q1 range:** raw **≈54508** (`ARTICULUS I.` / `QUAESTIO I.`, p.782) → ends before a1-q2 opener (raw ≈54670). Printed page begins **782** (offset pdf=printed+22; the QUAESTIO I body opens `Circa primum sic proceditur et quaeritur, utrum peccata parentum proximorum filiis imputentur…`). **HAND-OFF into d33-a1-q1:** NONE from the divisio footer band — both p.781 NOTAE AD COMMENTARIUM footers anchored in the divisio. The `ARTICULUS I.` header + the q1 body begin fresh on p.782 with their own footer sequence (restarts at 1; raw shows the p.782 footers `Exod. 20, 5` / `tertii Regum 14` / `Anselm. de Conceptu virginali` etc.). **VERIFY the Primo/Secundo-quaeritur alignment at promotion** (a1 has 2 questions: q1 = imputation/transmission of parents' actual sins; q2 = whether the first parent could transmit the justice of satisfaction). **d.33 commentary structure** (mirrors d.32 — 3 art × 2 q): ART I q1 (raw ≈54508) / q2 (raw ≈54670) → ART II q1 (raw ≈54863) / q2 (raw ≈55064) → ART III q1 (raw ≈55243) / q2 → DUBIA. Template = `vol2/bon-sent-II-d32-a1-q1.md` (quaestio) / `d33-divisio.md` (just-completed divisio). **d.33–40 skeletons were REGENERATED 2026-05-30 with a FIXED auto-chunker** (commits `95b6a45` tooling + `d5bec59` skeletons) — the d.32-class chunker bugs are now fixed, so the old `d33-divisio-dup2`/`d33-dubia-dup2` dups are GONE and every d.33–40 distinction has a clean 1 littera + 1 divisio + 1 dubia + correct quaestio count. **Still ALWAYS run the Primo/Secundo-quaeritur alignment check at promotion** (cheap insurance), but the gross structural misses are pre-cleared. Template = `vol2/bon-sent-II-d32-divisio.md` (divisio) / `d32-littera.md` (littera). **No decade polish until d.40.**
