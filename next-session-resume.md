# Next session — **d24-p2-dubia DONE 2026-05-29 (EXISTING skeleton rebuilt to Tier-2). DUBIA CIRCA LITTERAM MAGISTRI (Pars II), 3 DUB verified at 450 dpi, pp. 586–588, 16 apparatus defs [^1]–[^16], has_scholion=false, received p.586 footers ⁵+ as DUB I body [^1]–[^4]. → d.24 NOW FULLY TIER-2 (both Pars I and Pars II, all 17 chunks). Next distinction = d.25 (DISTINCTIO XXV at raw 41154).**
>
> **d.24 COMPLETE — verification:** all 17 d.24 chunks have `Phase C Tier 2 complete` status; paraphrase audit `--volume 2 --min-d 24 --max-d 24` = **0 critical / 0 high**; build 883 questions / 686 translated. (Header & apparatus audits skip Tier-2 chunks lacking frontmatter `line_start`/`line_end` — the corpus norm for promoted chunks, not a regression; the paraphrase audit is the reliable done-vs-skeleton signal.)
>
> ## NEXT TARGET = **`bon-sent-II-d25-p1-littera`** (MISSING — must be CREATED). Then d.25 Pars I commentary, then Pars II.
>
...
> - **LITTERA (Lombard):** Pars I = Cap. I–IV; Pars II = Cap. V–IX ("Pars II" / DIST. XXV. P. II at raw ~41210–41228). Two pars + separate COMMENTARIUS each ⇒ build **`d25-p1-littera`** (caps I–IV) + **`d25-p2-littera`** (caps V–IX), both **MISSING**. Verify cap boundaries at PDF before splitting.
