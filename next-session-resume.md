...
> # ★★★ THE ITINERARIUM IS THE ACTIVE FRONT. EIGHT OF TEN CHUNKS ARE TIER 2.
>
> ## ✅ `prol` · `capitula` · `c1` · `c2` · `c3` · `c4` · `c5` · **`c6` (2026-08-13, `3235541`)** — **pp. 295–312, zero `[?]` flags in all eight.** Remaining: **c7 · scholion.**
> ## ▶▶▶ **NEXT CHUNK = `bon-itin-c7`.** It opens **p. 312's LEFT column at ~39 % down**, heading + subtitle both in that column. Index claims pp. 312–313; **the claim is worth nothing** — close the end positively from the **`SCHOLION` heading**, which is c7's only closing witness (there is no `Cap. VIII.`).
>
> ## ★★ **THE Cap. VII TITLE QUESTION IS SETTLED — THE CAPITULA TABLE LOSES.**
> The in-place heading on p. 312 reads *De excessu mentali et mystico, in quo requies datur intellectui, **affectu totaliter in Deum per excessum transeunte**.* — **the VOLUME INDEX's form.** The capitula table's *affectu in Deum per excessum totaliter transeunte* is the variant. **The `WORKS` registry in `build-content.mjs` currently carries the TABLE's form and must be corrected to the heading's at c7**; the table's reading is transmitted text and belongs in c7's `## Notes`, not in the title.
>
> **Hand-off `bon-itin-c7` inherits from `bon-itin-c6` — re-derive it, do not adopt it:**
> - **p. 312 nn. 3–9 are c7's** (p. 312 holds **nine** notes; `KNOWN_TOTALS` is fed). n. 3 anchors at § 1's *ad sabbatum quietis*³, n. 4 at *Iesu Christo*⁴, n. 5 at *via et ostium*⁵ — **all three in the LEFT column**, though n. 5's own text prints in the left block too. **Blocks are 6 L / 3 R; verify every column yourself.**
> - **p. 312 n. 6 carries a gutter runover** (breaking at *…Exod. 12, 11: Et comedetis [agnum paschalem] festi-* | *nanter; est enim* phase*, id est transitus, Domini…*) — **c7 logs it.**
> - **⚠ Do NOT re-log p. 310 n. 7 or p. 311 n. 5** — both are in the ledger under `bon-itin-c6`.
> - **★ THE CAPITULUM BOUNDARY FALLS INSIDE p. 312's LEFT FOOTER BLOCK**, between nn. 2 and 3 — the same shape as p. 310, two leaves earlier. **Block, column and capitulum structure are three independent things.**
> - **p. 312 gutter = 1388**, confirmed at the band midpoint (zero band 1356–1420, centre rule at 1388–1389). **p. 313's default is 1161 on a 49 px run and is NOT settled** — the profile shows an ink island at 1149–1161 and a second at 1174–1179, so the band's extent is genuinely ambiguous there. **Take it to the direct body+footer profile before using it.**
> - **Marginalia on p. 312 that are c7's:** "Transitus supra se ipsum." · "Per Christum." · "Quomodo veniatur ad eam." and what follows.
> - **The Scholion (`bon-itin-scholion`, pp. 313–316) is the last chunk** and closes the work; *De reductione*'s half-title stands at p. 317/319, span still to be verified.
>
...
>
> ## ✅ Verification suite at the `bon-itin-c6` commit: `check-vol5-apparatus.py` **87 chunks / 818 entries** all passed (p. 312 correctly PENDING nn. 3–9; `KNOWN_TOTALS` fed through p. 312) · `check-vol5-census.py` rosters agree **87/87**, **seven** page-crossing runovers on the ledger (unchanged — c6 checked both its joints and found none) · `polish-style-scan --volume 5` CLEAN (87 files) · `build-content.mjs` **2020/2020, 6 books** · `build-citations.py`: c6 contributed **23 records, ZERO dangling and ZERO QA flags of its own**, corpus total unmoved at 226. ★ Two of c6's records are worth naming — the corrected *tom. I. pag. 57* resolves to `bon-sent-I-d2-a1-q4` and the corrected *Breviloq. p. II. c. 11* to `bon-brev-p2-c11`, which are exactly the targets the hand corroboration found. ⚠ QA job 0 (authority-preceded anaphora resolving to fabricated self-references) is UNCHANGED and still open.
