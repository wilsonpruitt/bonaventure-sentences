...
> 0 ahead, tree clean. NOTHING IS OWED.**
...
> 0 ahead, tree clean. NOTHING IS OWED.** The deploy boundary for this work is its **close at
...
> `8f54985`, 0 ahead, tree clean. NOTHING IS OWED.** The deploy
...
> ⚠ That live-state line is DATED and expires — never restate prod from a note.
>
> ## ⛔ READ BEFORE TOUCHING c4 — c3 CHANGED WHAT THE INDEX MEANS
> - **★ THE INDEX GIVES A HEADING PAGE, AND A HEADING PAGE MAY BE SHARED.** Collatio III does
>   **not** run 468–472 as the index pair (468, 473) implies — it runs **468–473**, because
>   `COLLATIO IV.`'s display heading stands **mid-leaf on p. 473**, below Collatio III's ¶¶ 18
>   and 19. **Never derive a span's last page by subtracting one from the next collatio's index
>   page.** Close every span positively on the plate. c4's own span is therefore **473–?**, and
>   its end must be fixed from `COLLATIO V.` (raw L79563, index p. 479) on the band.
> - **⚠ c4 INHERITS THREE NOTES AND MUST NOT RESTART p. 473 AT 1.** p. 473's footer register
>   numbers straight through the shared leaf: nn. 1–5 are Collatio III's, and **nn. 6, 7 and 8
>   are c4's** — n. 6 `Epist. II. Cor. 4, 6. — Seq. locus est Ps. 93, 10. — De hoc dono cfr.
>   III. Sent. d. 35. q. 2.` · n. 7 `Du Cange, Glossarium etc., notat, quod nomine clericorum
>   intelligantur viri litterati ac docti, vel etiam scholares.` · n. 8 `Cfr. III. Reg. 3, 7.
>   seqq.` **A hand-off tells you which notes are yours, never where they land — re-derive the
>   anchors off the band.** `check-vol5-apparatus.py` already reports p. 473 as
>   `PENDING n.6,7,8 -> not yet written`; that flag clears when c4 lands.
> - **p. 473's gutter is 1170, and `colcrop` reports it on a 16 px run** — one pixel above the
>   tool's hard-failure threshold — because the mid-leaf `COLLATIO IV.` heading destroys the run
>   inside the default 45–92 % window. Profiling **above** (0.08–0.28) and **below** (0.62–0.95)
>   the heading each gives a healthy 63–65 px band with the ink island at 1168–1173. The value is
>   right; only its run width was nonsense. Expect this at every mid-leaf collatio boundary.
> - **`Libr. <roman>. <ch>, <v>` is a genre trait, and the resolver now has a barrier for it.**
>   The reportatio names its book aloud and the note gives only the number. Three attested. If
>   you meet another, transcribe it as printed; `build-citations.py` will mark it unresolvable
>   rather than let a following `ibid.` bind to the previous note's book.
> - **★ THE OPENING-LEAF GUTTER RULE IS TWO-SIDED, and p. 468 is the second confirmation.**
>   The screen threw 370 px of drift and named its own outliers; three windows below the
>   Summarium reproduced `colcrop`'s default (1343, 56–59 px bands). Two of three openings have
>   now confirmed the default; only p. 457 corrected it. **Profile below the Summarium at every
>   opening and let the profile arbitrate — do not assume the default is wrong there.**
> - **Count the body against the Summarium, every time.** c1 (18/18), c2 (21/21) and c3 (19/19)
>   have all agreed. The c6/c14 short-Summarium mechanism has not fired in this work yet; it is a
>   property of the genre, not a spent one.
> - **⛔ `COLLATIO I.` CARRIES NO APPARATUS ANCHOR, and the question is CLOSED.** Settled at 10×
>   on p. 457. Nothing was looked for on p. 462 or p. 468 and nothing turned up. Do not reopen it.
>
> ## ⭐ One register question to settle at the c9 close, not before
> c3 renders *pietas* as **piety** (with pious / impious / impiety), because ¶¶ 17–18's argument
> needs one visible English root. c2 rendered *donum pietatis* "the gift of **godliness**" in four
...
## (superseded) previous START HERE — front was `bon-don-c3`, 2026-08-27
...
> Collatio II are both Tier 2** (c2 on 2026-08-27, `a88f83d`). ✅ **PUSHED 2026-08-27 —
> `origin/master` = `aba9077`, 0 ahead, tree clean. NOTHING IS OWED.** The deploy boundary for this
> work is its **close at c9**, so prod still serves the 2026-08-24 Hexaemeron deploy and nothing
> needs to ship yet.
> ⚠ That live-state line is DATED and expires — never restate prod from a note.
>
...
> The first attempt at pp. 447–449 numbered paragraphs 13–31 too low by several (`Ex parte
> occidentis...` is band-printed **27**, misread as `20` — the `2`/`7` class), which cascaded into
> mismatched apparatus (Reuben's etymology sitting under Aser's paragraph, etc.). **The whole span
> ¶¶ 12(cont.)–31 was re-read from the band a second time and cross-checked against the printed
> marginalia labels** (`Primo, de divino cultu`… `Quarto, de divino sensu`…), which is what actually
> caught it. **Lesson for the next long collatio: verify paragraph numbers against the marginalia
> labels as you go, not only at the end.**
>
> ## ★★ WHAT bon-hex-scholion INHERITS
> - **Nothing forwarded from c23's own apparatus** — c23's last page (449) closes cleanly on the
>   colophon and its own n. 10, with no runover into p. 450's register.
> - ⚠⚠ **FOUR OPEN `[?]` FLAGS IN c23, NOT THIS CHUNK'S TO FIX BUT WORTH KNOWING**: four scriptural
>   quotations in c23 ¶¶ 12–14 (`Paulus… utrum in corpore… an extra corpus`; `Signati estis spiritu
...
>   promised 30.
> - **Raw scouting only** (not fixed on the band): the Scholion runs roughly p. 450–454, and
>   *de septem donis* opens at **p. 457**. Fix every boundary positively on the band, including
>   whether the Additamentum (wherever it lands) sits before or after the Scholion's own close.
> - **pp. 450+ are NOT extracted.** `python3.11 tools/extract-pages.py --volume vol5 --pages 450-457
>   --dpi 450`. ⚠ `df -h /` first.
>
> ## ★★ What Collatio XXIII paid for — carry these
> - **★★★ THE PARAGRAPH-NUMBERING RE-DERIVATION ABOVE IS THE HEADLINE LESSON** — see above.
> - **★ A COLLATIO'S CLOSING COLOPHON CAN INTRODUCE A THIRD, UNPLANNED STRUCTURAL UNIT** — see the
>   Additamentum note above; the same discipline that caught the Vol V front-matter surprises
>   (Commentarius in Prologum, the sectio level in IV d.49) applies here: **the plate overrules the
>   plan, always check before writing the next chunk's frontmatter `division:`.**
> - **★ THE SUMMARIUM CAN OVERSHOOT, NOT ONLY UNDERSHOOT** — c6 ran three short, c14 one short, c21/c22
>   landed exact; c23's Summarium promised 30 and the body delivered 31. Never trust it as the count;
>   the Scholion's own forward citation was the tiebreaker here.
> - **★ REGISTER, reuse it (new for c23):** *cultus / nexus / zelus / sensus* → "worship / bond / zeal
>   / sense" (the four sides of the sealed city, each with three gates and three tribes) ·
>   *professio, veneratio, custoditio* → "profession, veneration, guarding" · *constantia, fiducia,
>   puritas* → "constancy, confidence, purity" · *condescensio, rectitudo, perpessio* →
>   "condescension, rectitude, endurance" (the three gates toward the north) · *perspicacitas,
>   tranquillitas, vivacitas* (desiderii/scrutinii/iudicii) → "liveliness, perspicacity, tranquillity"
>   · *asinus fortis* (Issachar) rendered literally as "a strong ass", not softened.
>
...
> rides with it. Push and deploy are both protected. **All twenty-three collationes are Tier 2;
> `bon-hex-scholion` is the last piece of the whole work, then the gate.**

---

# (superseded) `bon-hex-c23` — DONE 2026-08-22

> Note: this session replaced the prior `bon-hex-c23` START HERE block in place rather than
> archiving it verbatim below (a deviation from this file's usual practice); its content is
> superseded in full by the "`bon-hex-c23` IS TIER 2" block now at the top of this file, and by
> `vol5/bon-hex-c23.md`'s own `## Notes` section, commit `ae281fa`.
...
> Hexaemeron is **PARKED at `bon-hex-c21`**. Queue: ✅ Books II, III, IV → **Book I's family (7 of 8
> units done)** → Hexaemeron c21–c23 + Scholion → work-close gate → deploy.
>
> ## ✅ `bon-sent-I-capitula` IS TIER 2 — pp. 17–**22**, 209 chapter titles, 77 apparatus entries, zero `[?]` (2026-08-20)
> p. 17 nn. 15–22 (inherited) · p. 18 nn. 1–19 · p. 19 nn. 1–15 · p. 20 nn. 1–16 · p. 21 nn. 1–13 ·
> **p. 22 nn. 1–6 only.** Build **2059 → 2060**, all translated; marker pairing **77 / 77 / 77 in
> identical order**; structural parity 207 numbered titles + 2 unlabelled in each language;
> `polish-style-scan` unchanged at the same **10 PAIR issues across 5 chunks, all pre-existing**;
> `build-citations.py` QA flags **202 → 201** — see the trap below, the decrease is not good news.
> Commit `9e1181c`. ⚠ Nothing pushed; push and deploy are protected.
>
> ## ★★★ THE TABLE RAN ONTO p. 22 — THE LAST NOTE SAID "~p. 21" AND EVERY EARLIER ONE SAID pp. 17–21.
> dd. XLV c. III – XLVIII print at the **top third of p. 22**, in both columns, and the
> `COMMENTARIUS IN PROLOGUM MAGISTRI.` heading stands below them on the same leaf. The projection
> came from the raw, where p. 22's running head (`COMMENTARIUS IN PROLOGUM.`) sits *above* the
> table's last entries and reads exactly like a boundary. **It is a running head.** Book I's table
> prints **no `EXPLICIT` formula at all** — the third distinct answer in three books (II closes on
> `EXPLICIUNT`, IV prints none, I prints none) — and was closed positively on the display heading.
>
> ## ⚠⚠ HAND-OFF INTO `bon-sent-I-prol-comm`: **p. 22's SECOND REGISTER IS YOURS, ALL SIX OF IT.**
> **p. 22 carries TWO independently numbered footer registers on one leaf, and both start at 1.** The
> left block runs the capitula's nn. 1–6, then prints the sub-heading **`NOTAE AD COMMENTARIUM.`** in
> spaced caps, under which a **fresh sequence begins again at n. 1** and runs **nn. 1–2 in the left
> block, nn. 3–6 in the right**. Those six are yours. **The printed sub-heading is the only tell** —
> nothing in the numbering says a new sequence has begun, and a reader dividing the register by
> number would take twelve notes and pair four of them to nothing.
> ⚠ **Ownership is all that is forwarded.** The anchors *appear* to fall at `moverunt`¹, `ibi`²,
> `tangit`³, `ipsum`⁴, `dicit`⁵, `virtutis`⁶ — but that was read at whole-page scale for structure,
> and n. 6 (*Minus bene et reluctantibus mss. cum ed. 1, hic Vat. addit* indicium) sits oddly against
> `virtutis`. **Re-derive every position and column at the band; adopt none of it.**
>
> ## ★★ p. 22's TABLE GUTTER IS **1060** AND THAT IS NOT YOUR GUTTER.
> The leaf stacks **four regions** — the chapter table (two columns), the full-measure display
> heading, the Commentarius body (two columns, different measure), and the footer register. 1060 was
> measured over the **table rows only** (`gutter-profile.py 22 0.065 0.330 vol1`; band 1036–1085,
> rule 1058–1063). **Profile the body region you are transcribing** — this is the `bon-itin-c7`
> rule, and p. 22 is a textbook instance of it. ⚠ And on p. 22 the row windows are useless
> (1035–1106 on 140–281 px runs); take the rule from the direct ink profile, as on every leaf of the
> table. p. 22 is **already extracted at 450 dpi**; **pp. 23–25 are not.**
>
> ## ⚠ `divisio` AND `dubia` ARE STILL NOT ADMITTED BY `SENTENCES_FRONT_TYPES`
> in `site/scripts/build-content.mjs`. This unit needs them — **add them when the unit is read at the
> plate, and title them "Prologus …", not "Proemium …".** `littera` and `capitula` are already
> admitted and verified live. ★ The trap that change walked past: `chunkMeta.type` defaults to
> `"quaestio"` when a file declares none, and `vol1/bon-sent-I-proleg.md` — a bare 77k-word OCR dump
> — declares none. **The guard tests `declaredType`, the raw frontmatter value. Do not collapse
> `declaredType` back into `type`.**
>
> ## ★★★ THE TRAP THIS SESSION FOUND, AND IT IS GENERAL: **A FLAG THAT *DISAPPEARS* ON YOUR COMMIT.**
> `bon-sent-I-d25-littera` `[^17]` cites `Cfr. supra pag. 19, nota 3` about 1 John 5:7 and cod. C's
> *dant* for *perhibent*. **p. 19 n. 3 — read off the band this session — is about something else
> entirely.** The note Quaracchi points at is **`bon-sent-I-d2-littera` `[^57]`**, whose whole subject
> is that variant, on **pp. 46–49**: the reading is almost certainly **`pag. 49`**, the chronic `1`/`4`
> serif class. **Before this session that citation resolved `dangling` — a visible QA line. The moment
> `bon-sent-I-capitula` claimed p. 19, the same wrong citation resolved to `chunk`, cleanly, and the
> corpus flag count went DOWN by one.** It will now render as a *Cited by* card on the capitula page
> and read to every later session as settled. **Corollary: on any chunk that closes a gap in a page
> range, the flags that vanish deserve the same look as the ones that appear** — reporting "none
> attributable to my chunk" does not catch it, because the count attributable to it is zero.
> **Not corrected: d.25's plate was not opened.** Hand it to whoever works d.25's littera; `49` still
> needs its `nota 3` confirmed against p. 49's register.
>
> ## ★★★ BOOK I's FRONT IS **EIGHT UNITS ACROSS pp. 1–25** — SEVEN ARE DONE
...
> | `bon-sent-I-littera` — PROLOGUS MAGISTRI | 16–17 | 11102–11282 | ✅ done |
> | `bon-sent-I-capitula` — INCIPIUNT CAPITULA PRIMI LIBRI | 17–**22** | 11285–~12253 | ✅ **done** |
> | `bon-sent-I-prol-comm` — COMMENTARIUS IN PROLOGUM MAGISTRI (DIVISIO TEXTUS + DUBIA, three parts) | **22**–25 | 12255–~12779 | ▶ **next** |
>
> Landmarks, absolute raw lines: `COMMENTARIUS IN PROLOGUM MAGISTRI.` **12255** (the display heading)
> · `DIVISIO TEXTLIS.` **12259** (raw garble for `DIVISIO TEXTUS`) · `DUBI.V CIRCA PRIM.\M PARTEM
> PROLOGI MAGISTRI` ~**12290** · `DUB. I.` ~**12292**. The bare `COMMENTARIUS IN PROLOGUM.` at
> **12197, 12355, 12515, 12657** are **running heads** (pp. 22, 23, 24, 25) — the same class that
> mis-set this chunk's span. **Fix every boundary positively on the band. The far end is p. 25, and
> what follows is `bon-sent-I-d1-commentary` — see the mis-paging defect below.**
>
> ## ⚠ THE RAW IS ROTTED ACROSS THIS UNIT'S OPENING AND THE VOL I "OCR IS AUTHORITATIVE" RULE IS SPENT HERE
> The lines immediately above the display heading read `Dous soit`, `Doi ■iolnntas`, `nomino
> voluntolis iilorduni`, `Utruni`, `\elit`, `lioi-i`. **The body below the heading recovers
> considerably** (the `DIVISIO TEXTUS` paragraph transcribes cleanly in the raw) — **so grade the raw
> per region and per column-run, not per page**, and give the verdict for body and footer separately.
> Apparatus is bands-only regardless.
>
...
> and its `printed_pages: [1] / pdf_pages: [103]` is simply wrong. **pp. 26–29 are not extracted and a
> page assignment is fixed on the plate.** ★ It is now the chunk *immediately after* `-prol-comm`, so
> whoever closes p. 25 is the natural person to close this too.
>
...
> whole-page read is for STRUCTURE only — every one of this chunk's 77 apparatus entries was read off
> a 2.2–2.6× `tools/footcrop.py` band, and the four defective sorts needed 3.6–5×.** Offset
> `pdf = printed + 102`. **pp. 1–22 are extracted; pp. 23 onward are not.**
> ★ `footcrop.py` takes `<page> <split_x> [row_lo] [row_hi] [scale] [volume]` and is the right tool
> for **any** region crop on a stacked leaf, not only footers — it is what read this table.
>
> ## ⚠ STILL OWED BEFORE/WITH THE NEXT DEPLOY
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
> 2. The About page's **"What Is Known to Be Wrong"** — Wilson's to frame.
> 3. **Book I's last front-matter unit** (`-prol-comm`), per the ruling above.
...
> 5. **New:** `bon-sent-I-d25-littera` `[^17]`'s `pag. 19` → almost certainly `pag. 49` (above).
>
> ## Corpus state (verified in-session 2026-08-20)
> `build-content.mjs` **2060/2060, 8 books** · `build-citations.py` QA flags **201** ·
> `polish-style-scan` **10 PAIR issues / 5 chunks, all pre-existing.**
> ⚠ Nothing pushed; push and deploy are protected.
...
> ## ⛔⛔ THE ORDER — WILSON'S RULING, 2026-08-20. UNCHANGED.
> **ALL FOUR SENTENCES PROEMIA MUST BE TIER 2 BEFORE THE NEXT DEPLOY — BOOK I INCLUDED.** The
> Hexaemeron is **PARKED at `bon-hex-c21`**. Queue: ✅ Books II, III, IV → **Book I's family (5 of 7
> units done — all four proemium quaestiones are now Tier 2)** → Hexaemeron c21–c23 + Scholion →
> work-close gate → deploy.
>
> ## ✅ `bon-sent-I-proem-q4` IS TIER 2 — pp. 14–15, 21 apparatus entries, zero `[?]` (2026-08-20)
> p. 14 nn. 1–10 (inherited) · p. 15 nn. 1–11 · **nothing forwarded.** Build **2057 → 2058**, all
> translated; `polish-style-scan` full corpus: the same **10 PAIR issues across 5 chunks, all
> pre-existing**; `build-citations.py` QA flags **202 → 202, none attributable to this chunk** (it
> contributes no citation record at all — its apparatus cites only Aristotle, an excluded authority,
> and Quaracchi's own *In Prologo* / *In Prooemio*). ⚠ Nothing pushed; push and deploy are protected.
>
> ## ⛔ q4 PRINTS NO SCHOLION, AND THAT IS A POSITIVE FINDING
> q1, q2 and q3 each carry one; **q4 does not.** p. 15's body ends two-fifths down the leaf, the
> lower half of both columns is blank but for the ornamental rule and the footer register, and the
> next display matter is `PROLOGUS MAGISTRI` on p. 16. **Do not read a missing Scholion in this
> family as a failed read — but do fix it positively from the next display heading, never from the
> white space.**
>
> ## ★★★ BOOK I's FRONT IS **SEVEN UNITS ACROSS pp. 1–25** — FIVE ARE DONE
> | unit | printed pp. | raw lines | status |
> |---|---|---|---|
...
> | `bon-sent-I-proem-q2` — QUAESTIO II, *Quae causa formalis quive modus procedendi* | 9–12 | 10234–10626 | ✅ **done** |
> | `bon-sent-I-proem-q3` — QUAESTIO III, *Utrum … contemplationis gratia, an ut boni fiamus* | 12–14 | 10627–10906 | ✅ **done** |
> | `bon-sent-I-proem-q4` — QUAESTIO IV, *Quae sit causa efficiens sive auctor huius libri* | 14–15 | 10907–11101 | ✅ **done** |
> | `bon-sent-I-littera` — PROLOGUS MAGISTRI, *Cupientes aliquid* | 16–17 | 11102–~11283 | ▶ **next** |
> | `bon-sent-I-capitula` — INCIPIUNT CAPITULA PRIMI LIBRI | 17–21 | 11285–~12190 | to build |
> | `bon-sent-I-prol-comm` — COMMENTARIUS IN PROLOGUM MAGISTRI (divisio + dubia, three parts) | 22–25 | ~12197–12779 | to build |
>
> Landmarks, absolute raw lines: `PROLOGUS MAGISTRI` **11102** (raw prints `PR0L0GU8 MAGI8TRI`) ·
> `Explicit prologus.` ~**11283** · `INCIPIUNT CAPITULA PRIMI LIBRI SENTENTIARUM` ~**11285**.
> **Every span still gets fixed positively on the plate.** pp. 1–15 are extracted at 450 dpi;
> **p. 16 onward are not.**
>
> ## ⚠⚠ THE LITTERA'S RAW IS MARGINALIA-CONTAMINATED — THE VOL I "OCR IS AUTHORITATIVE" RULE IS
> **DEGRADED ON THIS UNIT.** Lombard's prologue is set with a dense outer-margin gloss apparatus, and
> the raw splices it into the body line by line: `1™%^*°-`, `haeretka™ *^"'"`, `praviuitis`,
> `movent ad`, `''imToc"`, `"■""'^` all stand *inside* the opening sentences. **Trim them to the
> Marginalia list and re-set the affected lines from the plate** — this is the same species as q3's
> p. 14 Scholion continuation, but it runs the whole unit rather than one region.
>
> ## ⚠ HAND-OFF INTO `bon-sent-I-littera`: **q4 FORWARDS NOTHING.**
> p. 15's whole register — all eleven notes — answers to anchors in q4's body, verified there by
> anchor. p. 16 is a fresh leaf and its footer is entirely the littera's to read.
>
> ## ★★ BLOCK vs ANCHOR: THEY COINCIDED ONCE IN q4 AND CAME APART AGAIN IN THE SAME CHUNK
> p. 14: block **7/3** (n. 7 overrunning the column division) against anchors **7/3** — *the first
> agreement in this family.* p. 15: block **6/5** against anchors **5/6** (n. 6 anchors at
> *sic ⁶ in proposito,* in the right column). **One leaf's agreement licenses nothing.**
>
> ## ★★ THE RUNNING HEAD'S LYING RUN ENDED AT p. 14 — DON'T START TRUSTING IT
> p. 14's `PROOEMII QUAEST. IV.` is right about q4 and was wrong about q3 on the same leaf; p. 15's
> is simply right. Nothing was set from either. Expect p. 16's head to be worth nothing either way.
>
> ## ⚠ DIGIT AND SIGLUM CLASSES ON THESE LEAVES — DON'T CALIBRATE THE NEXT ONE ON THEM
> The `1`/`4` class fired in both directions **on one line** (p. 14 n. 3: `c. 14.` with a serif 1
> beside `Tract. 4.` with a true 4). **The edition siglum is the chronic case:** `ed. 1` prints a
> serif `1` that the raw renders now `I`, now `4` — two of p. 15's notes read `ed. 4` in the raw and
> `ed. 1` on the plate. Sigla corrected: raw `FHIOTH` → **`F H I O T U`**, raw `BCDGHIvOSUV` →
> **`B C D G H K O S U V`** (raw `Iv` = `K`). **Read every citation run and every siglum run at 2.2×
> or better.**
>
> ## ⛔ TWO DEFECTIVE SORTS ON THESE LEAVES, HANDLED DIFFERENTLY — READ q4's `## Notes` BEFORE
> **DECIDING A THIRD.** p. 14 prints `«pau` / `isper` across a line break with neither `l` nor hyphen
> (transcribed as printed, `pauisper`, for *paulisper*); p. 15 n. 10 prints `sent·ntias` with the `e`
> inked only at its top (transcribed *sententias,* because a half-inked letter is a defect of this
> copy, not a reading). Both are documented in the chunk; neither is flagged.
>
> ## ⛔⛔ NEVER RUN `audit-paraphrase.py` ON A FRONT-MATTER RANGE — IT DESTROYS A REAL LOG
> All three guard-rail audits select on `bon-sent-{I,II,III,IV}-d(\d+)-` and print a clean verdict on
> **0 chunks audited** for a `distinctio: 0` chunk. **New at q4:** `audit-paraphrase.py --min-d 0
> --max-d 0` *also overwrote* `manual-review/d1-d40-paraphrase-audit.md` with an empty 0-chunk report,
> silently discarding the last real Vol I paraphrase audit (restored with `git checkout --`). Verify a
> front-matter chunk by marker pairing + structural parity + `polish-style-scan` + `build-citations` +
> plate discipline, and say so.
>
> ## ✅ `build-content.mjs` ALREADY PUBLISHES A PROEMIUM QUAESTIO — nothing further is needed
> `SENTENCES_FRONT_TYPES` admits **`quaestio`**, titled **`Proemium, Q. N`**. Verified live at q1–q4.
...
> ## ⚠ VOL I PLATES: 450 dpi IS NOT WHAT YOU GET, AND THAT IS FINE
> `extract-pages.py --volume vol1 --dpi 450` produces **2028 × 2986 px, ~1.3–2.4 MB** pages — the
> vol1 PDF's own raster caps it. They are **readable whole, directly, under the API image cap**; no
> `colcrop` has been needed anywhere in this front. **But a whole-page read is for STRUCTURE only —
> every apparatus entry in q4 was read off a 2.2× PIL crop of its footer band.** Offset
> `pdf = printed + 102`.
>
> ## ⚠ STILL OWED BEFORE/WITH THE NEXT DEPLOY
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
...
>
> ## Corpus state (verified in-session 2026-08-20)
> `build-content.mjs` **2058/2058, 8 books** · `build-citations.py` QA flags **202** ·
> `polish-style-scan` **10 PAIR issues / 5 chunks, all pre-existing.**
> ⚠ Nothing pushed; push and deploy are protected.
...
> # (superseded — DONE 2026-08-20) **`bon-sent-I-proem-q4`** (QUAESTIO IV, *Quae sit causa efficiens sive auctor huius libri*), which opens **just below the middle of p. 14** at the bare `QUAESTIO IV.` display heading, raw L10907.
>
> ## ⛔⛔ THE ORDER — WILSON'S RULING, 2026-08-20. UNCHANGED.
> **ALL FOUR SENTENCES PROEMIA MUST BE TIER 2 BEFORE THE NEXT DEPLOY — BOOK I INCLUDED.** The
> Hexaemeron is **PARKED at `bon-hex-c21`**. Queue: ✅ Books II, III, IV → **Book I's family (4 of 7
> units done)** → Hexaemeron c21–c23 + Scholion → work-close gate → deploy.
>
> ## ✅ `bon-sent-I-proem-q3` IS TIER 2 — pp. 12–14, 15 apparatus entries, zero `[?]` (2026-08-20)
> p. 12 nn. 1–6 (inherited) · p. 13 nn. 1–9 · **p. 14 none — all ten of its notes are q4's.** Build
> **2056 → 2057**, all translated; `polish-style-scan` full corpus: the same **10 PAIR issues across
> 5 chunks, all pre-existing**; `build-citations.py` QA flags **205 → 202** — four page-ownership
> flags in Vols II and IV cleared (they cite `tom. I. pag. 13, nota 5` and `pag. 14, nota 7`), one
> author-governance false positive added (Albert's `I. Sent. d. 1. a. 4.`), dispositioned in the
> chunk's `## Notes`. Commit `78497e4`. ⚠ Nothing pushed; push and deploy are protected.
>
> ## ★★★ BOOK I's FRONT IS **SEVEN UNITS ACROSS pp. 1–25** — MAPPED, AND ONE ROW NOW CORRECTED
> | unit | printed pp. | raw lines | status |
> |---|---|---|---|
> | `bon-sent-I-proem` — the proemium on Job 28:11 | 1–6 | 8896–9686 | ✅ **done** |
> | `bon-sent-I-proem-q1` — QUAESTIO I, *Quae sit materia quodve subiectum* | 6–9 | 9694–10233 | ✅ **done** |
> | `bon-sent-I-proem-q2` — QUAESTIO II, *Quae causa formalis quive modus procedendi* | 9–12 | 10234–10626 | ✅ **done** |
> | `bon-sent-I-proem-q3` — QUAESTIO III, *Utrum … contemplationis gratia, an ut boni fiamus* | 12–14 | 10627–10906 | ✅ **done** |
> | `bon-sent-I-proem-q4` — QUAESTIO IV, *Quae sit causa efficiens sive auctor huius libri* | **14–15** | 10907–11101 | ▶ **next** |
> | `bon-sent-I-littera` — PROLOGUS MAGISTRI, *Cupientes aliquid* | 16–17 | 11102–11283 | to build |
> | `bon-sent-I-capitula` — INCIPIUNT CAPITULA PRIMI LIBRI | 17–21 | 11285–~12190 | to build |
> | `bon-sent-I-prol-comm` — COMMENTARIUS IN PROLOGUM MAGISTRI (divisio + dubia, three parts) | 22–25 | ~12197–12779 | to build |
>
> **⚠ THE MAP WAS WRONG ABOUT ONE THING AND IT IS NOW FIXED: Quaestio IV opens on p. 14, not p. 15.**
> Earlier rows read "?–15" for q4 because q3's end had never been fixed on a plate. q3's Scholion
> fills only the upper third of p. 14 and `QUAESTIO IV.` stands below it on the same leaf. Landmarks,
> absolute raw lines: `QUAESTIO IV.` **10907** · `PROLOGUS MAGISTRI` **11102** · `Explicit prologus.`
> ~**11283**. **Every span still gets fixed positively on the plate.** pp. 1–14 are extracted at
> 450 dpi; **p. 15 onward are not.**
>
> ## ⚠ HAND-OFF INTO `bon-sent-I-proem-q4` — ownership verified, positions NOT
> **ALL TEN of p. 14's notes are q4's.** Verified at the plate by anchor: n. 1 glosses q4's opening
> *potest quaeri,* and nn. 2–10 answer to anchors in its fundamenta, its *Contra* and the opening of
> its *Respondeo* (videtur ² · de Magistro ³ · veteri ⁴ · in littera ⁵ · Si tu dicis ⁶ · denominatio ⁷ ·
> auctorem ⁸ · in littera ⁹ · aliena ¹⁰). **Where they land is yours to re-derive** — the ownership
> read was taken off a whole-page view, which is for STRUCTURE only. q3 forwards nothing else and
> owns nothing in p. 14's register.
>
> ## ★★ THE RUNNING HEAD HAS NOW LIED ON FOUR CONSECUTIVE LEAVES OF THIS FAMILY
> p. 9 `QUAEST. II.` above a leaf mostly q1's · p. 12 `QUAEST. III.` above a leaf whose top half is
> q2's · **p. 14 `QUAEST. IV.` above a leaf whose top third is q3's Scholion.** Expect p. 15's head to
> be worth nothing either way. Fix boundaries from the display headings only.
>
> ## ★★ A SCHOLION CARRIES NO ANCHORS — THREE TIMES RUNNING, SAME REASON
> q1 owned none of p. 9's four notes, q2 none of p. 12's six, q3 none of p. 14's ten. **Read anchors,
> only anchors**; where the text sits on the page decides nothing.
>
> ## ⚠ THE RAW IS NOT AUTHORITATIVE ON A SCHOLION CONTINUATION LEAF
> p. 14's Scholion III–IV continuation is cascade-shattered in the raw by two-column bleed
> (`I. Ulei-que aclmillil`) and had to be re-set from the plate — the one place in Book I's front
> where the standing Vol I "OCR is authoritative" rule failed. **Expect the same on any leaf where a
> Scholion runs across the top of a page in small type.**
>
> ## ⚠ DIGIT CLASSES ON THESE LEAVES — DON'T CALIBRATE THE NEXT ONE ON THEM
> The `3`/`5` class fired once (`III. Sent. d. 35.` for raw `d. 33.` — the same citation q2 had to
> correct). The `1`/`4` class is **live in the Scholion citation runs and invisible at 2×**: Scholion
> IV's `q. 1. m. 1. et 2.` (serif 1) stands beside `Scot. … q. 4.` (true 4) in one line. **Read every
> citation run at 4×.**
>
> ## ⛔ A QUARACCHI MISPRINT NOW STANDS IN THE CORPUS AS PRINTED — DO NOT "FIX" IT
> p. 12 n. 5 prints **`Scholaticorum`** for *Scholasticorum,* while p. 13 n. 6 on the facing leaf
> prints it correctly. Both confirmed at 2.2×; Quaracchi print no note. Same discipline as q2's
> `proprie distinguntur` / `thelogorum`.
>
> ## ✅ `build-content.mjs` ALREADY PUBLISHES A PROEMIUM QUAESTIO — nothing further is needed
> `SENTENCES_FRONT_TYPES` admits **`quaestio`**, titled **`Proemium, Q. N`**. Write the chunk with
> `type: quaestio`, `distinctio: 0`, `quaestio: N`. Verified live at q1, q2 and q3.
>
> **★★ THE TRAP THAT CHANGE WALKED PAST.** `chunkMeta.type` defaults to `"quaestio"` when a file
> declares none, and `vol1/bon-sent-I-proleg.md` — a bare **77k-word OCR dump** — declares none. The
> guard tests **`declaredType`**, the raw frontmatter value. **Do not collapse `declaredType` back
...
>
> ## ⚠ VOL I PLATES: 450 dpi IS NOT WHAT YOU GET, AND THAT IS FINE
> `extract-pages.py --volume vol1 --dpi 450` produces **2028 × 2986 px, ~2.0–2.3 MB** pages — the
> vol1 PDF's own raster caps it. They are **readable whole, directly, under the API image cap**; no
> `colcrop` has been needed anywhere in this front. **But a whole-page read is for STRUCTURE only —
> every apparatus entry in q3 was read off a 2.2–2.4× PIL crop of its footer band.** Offset
> `pdf = printed + 102`.
>
...
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
> 2. The About page's **"What Is Known to Be Wrong"** — Wilson's to frame.
> 3. **Book I's remaining three front-matter units**, per the ruling above.
> 4. **Forwarded defect, still nobody's fix:** Quaracchi cite `IV. Sent. d. 2. p. I.` (three chunks
>    do) but the corpus's Book IV d. 2 has no pars chunks. Hand it to whoever works Book IV's body.
>
> ## Corpus state (verified in-session 2026-08-20)
> `build-content.mjs` **2057/2057, 8 books** · `build-citations.py` QA flags **202** ·
> `polish-style-scan` **10 PAIR issues / 5 chunks, all pre-existing.**
> ⚠ Nothing pushed; push and deploy are protected.

---

# (superseded) `bon-sent-I-proem-q3` — DONE 2026-08-20


> # (superseded — DONE 2026-08-20) **`bon-sent-I-proem-q3`** (QUAESTIO III, *Utrum hic liber sive theologia sit contemplationis gratia, an ut boni fiamus, sive utrum sit scientia speculativa, an practica*), which opens **at the middle of p. 12** at the bare `QUAESTIO III.` display heading, raw L10627.
>
> ## ⛔⛔ THE ORDER — WILSON'S RULING, 2026-08-20. UNCHANGED.
> **ALL FOUR SENTENCES PROEMIA MUST BE TIER 2 BEFORE THE NEXT DEPLOY — BOOK I INCLUDED.** The
> Hexaemeron is **PARKED at `bon-hex-c21`**. Queue: ✅ Books II, III, IV → **Book I's family (3 of 7
> units done)** → Hexaemeron c21–c23 + Scholion → work-close gate → deploy.
>
> ## ✅ `bon-sent-I-proem-q2` IS TIER 2 — pp. 9–12, 26 apparatus entries, zero `[?]` (2026-08-20)
> p. 9 nn. 1–4 (inherited) · p. 10 nn. 1–9 · p. 11 nn. 1–13 · **p. 12 none — see below.** Build
> **2055 → 2056**, all translated; `build-citations.py` QA flags **207 → 205**, the chunk's own two
> being author-governance false positives (Albert's `I. Sent. d. 1. a. 5.` and Scotus's `II. Sent.
> d. 1. q. 4.`, both dispositioned in its `## Notes`); `polish-style-scan` full corpus: the same
> **10 PAIR issues across 5 chunks, all pre-existing**. Commit `ea146d6`.
> ⚠ Nothing pushed; push and deploy are protected.
>
...
> | `bon-sent-I-proem-q1` — QUAESTIO I, *Quae sit materia quodve subiectum* | 6–9 | 9694–10233 | ✅ **done** |
> | `bon-sent-I-proem-q2` — QUAESTIO II, *Quae causa formalis quive modus procedendi* | 9–12 | 10234–10626 | ✅ **done** |
> | `bon-sent-I-proem-q3` — QUAESTIO III, *Utrum … contemplationis gratia, an ut boni fiamus* | 12–? | 10627–10906 | ▶ **next** |
> | `bon-sent-I-proem-q4` — QUAESTIO IV | ?–15 | 10907–11101 | to build |
> | `bon-sent-I-littera` — PROLOGUS MAGISTRI, *Cupientes aliquid* | 16–17 | 11102–11283 | to build |
> | `bon-sent-I-capitula` — INCIPIUNT CAPITULA PRIMI LIBRI | 17–21 | 11285–~12190 | to build |
> | `bon-sent-I-prol-comm` — COMMENTARIUS IN PROLOGUM MAGISTRI (divisio + dubia, three parts) | 22–25 | ~12197–12779 | to build |
>
> Landmarks, absolute raw lines: `QUAESTIO IV.` **10907** · `PROLOGUS MAGISTRI` **11102** ·
> `Explicit prologus.` ~**11283**. **Every span still gets fixed positively on the plate; these are
> search keys, not boundaries.** pp. 1–12 are extracted at 450 dpi; **p. 13 onward are not.**
>
> ## ⚠ HAND-OFF INTO `bon-sent-I-proem-q3` — ownership verified, positions NOT
> **p. 12's whole register — nn. 1–6 — is q3's**, verified at the plate by anchor: n. 1 glosses q3's
> opening *Tertio quaeritur,* and nn. 2–6 answer to anchors in its fundamenta and *sed contra*
> (verae ² · undecimo ³ · in littera ⁴ · de fide ⁵ · ab opere nostro ⁶). Re-derive where they land;
> a hand-off tells you which notes are yours and never where they land. The register is split
> **5/1 by block** (nn. 1–5 left, n. 6 right, with n. 5 overrunning the column division) against
> **3/3 by anchor** (nn. 1–3 in the left column, nn. 4–6 in the right) — that is a claim to
> re-check, not a fact to adopt.
> **q2 forwards nothing else**: it owns no apparatus on p. 12 at all.
>
> ## ★★ THE RUNNING HEAD LIED TWICE IN ONE CHUNK — IN BOTH DIRECTIONS
> p. 9's head reads `PROOEMII QUAEST. II.` above a leaf whose bottom third only is q2's; **p. 12's
> head reads `PROOEMII QUAEST. III.` above a leaf whose top half is q2's Scholion.** The boundary is
> fixed by the signature line + printed page number at one end and by the `QUAESTIO III.` display
> heading at the other. Same asymmetric-witness rule as Vol V's `Cap. N.` heads. **Expect p. 15's
> head to name Quaestio IV or the Prologue and to be worth nothing either way.**
>
> ## ★★ A CHUNK CAN OWN A PRINTED PAGE AND HAVE NO APPARATUS ON IT — NOW TWICE IN THIS FAMILY
> q1 owned none of p. 9's four notes; q2 owns none of p. 12's six. **Both times the reason is the
> same: a Quaracchi Scholion carries no anchors at all, citing its authorities inline.** Read
> anchors, only anchors — where the text sits on the page decides nothing.
>
> ## ★ BLOCK STRUCTURE AND ANCHOR STRUCTURE CAME APART ON BOTH OF q2's FOOTER PAGES
> p. 10: block **4/5**, anchors **7/2**. p. 11: block **9/4** (n. 9 overrunning into the right
> block), anchors **6/7**. p. 9's n. 1 likewise overruns the column division. State the two splits
> separately in every `## Notes`.
>
> ## ⚠ DIGIT CLASSES ON THESE LEAVES — DON'T CALIBRATE THE NEXT ONE ON THEM
> The `3`/`5` class fired twice (`6569` for raw `6369`; `III. Sent. d. 35.` for raw `d. 33.`, and
> the corrected form is the one that resolves), the chronic raw **R-for-K** twice (`Cod. K`), and
> the `1`/`4` class **not at all** — though Scholion I's `S. I. q. 4. a. 2.` was checked at 4× and
> **prints 4**; raw agrees; transcribed as printed.
>
> ## ✅ `build-content.mjs` ALREADY PUBLISHES A PROEMIUM QUAESTIO — nothing further is needed
> `SENTENCES_FRONT_TYPES` admits **`quaestio`**, titled **`Proemium, Q. N`**. Write the chunk with
> `type: quaestio`, `distinctio: 0`, `quaestio: N`. Verified live at q1 and again at q2.
>
> **★★ THE TRAP THAT CHANGE WALKED PAST.** `chunkMeta.type` defaults to `"quaestio"` when a file
> declares none, and `vol1/bon-sent-I-proleg.md` — a bare **77k-word OCR dump** — declares none. The
> guard tests **`declaredType`**, the raw frontmatter value. **Do not collapse `declaredType` back
...
> `colcrop` has been needed anywhere in this front. **But a whole-page read is for STRUCTURE only —
> every apparatus entry in q2 was read off a 1.7–2.0× PIL crop of its footer band.** Offset
> `pdf = printed + 102`.
>
> ## ⛔ THE THREE GUARD-RAIL AUDITS CANNOT SEE A FRONT-MATTER CHUNK
> They select on `bon-sent-{I,II,III,IV}-d(\d+)-`; a distinctio-0 chunk never matches and all three
...
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
> 2. The About page's **"What Is Known to Be Wrong"** — Wilson's to frame.
> 3. **Book I's remaining four front-matter units**, per the ruling above.
> 4. **Forwarded defect, still nobody's fix:** Quaracchi cite `IV. Sent. d. 2. p. I.` (three chunks
>    do) but the corpus's Book IV d. 2 has no pars chunks. Hand it to whoever works Book IV's body.
>
> ## Corpus state (verified in-session 2026-08-20)
> `build-content.mjs` **2056/2056, 8 books** · `build-citations.py` QA flags **205** ·
> `polish-style-scan` **10 PAIR issues / 5 chunks, all pre-existing.**
> ⚠ Nothing pushed; push and deploy are protected.

---

# (superseded) `bon-sent-I-proem-q2` — DONE 2026-08-20
...
> ## ⛔⛔ THE ORDER — WILSON'S RULING, 2026-08-20. UNCHANGED.
> **ALL FOUR SENTENCES PROEMIA MUST BE TIER 2 BEFORE THE NEXT DEPLOY — BOOK I INCLUDED.** The
> Hexaemeron is **PARKED at `bon-hex-c21`**. Queue: ✅ Books II, III, IV → **Book I's family (1 of 7
> units done)** → Hexaemeron c21–c23 + Scholion → work-close gate → deploy.
>
> ## ✅ `bon-sent-I-proem` IS TIER 2 — pp. 1–6, 81 apparatus entries, zero `[?]` (2026-08-20)
> p. 1 nn. 1–12 · p. 2 nn. 1–19 · p. 3 nn. 1–16 · p. 4 nn. 1–15 · p. 5 nn. 1–17 · **p. 6 nn. 1–2
> only.** Build **2053 → 2054**; `build-citations.py` QA flags **211 → 207** — the chunk contributes
> **zero** flags of its own across **36 records** and retires four dangling references.
> `polish-style-scan` full corpus: the same **10 PAIR issues across 5 chunks, all pre-existing**
...
> |---|---|---|---|
> | `bon-sent-I-proem` — the proemium on Job 28:11 | 1–6 | 8896–9686 | ✅ **done** |
> | `bon-sent-I-proem-q1` … `-q4` — QUAESTIONES PROOEMII I–IV | 6–15 | 9694–~11100 | ▶ **next** |
> | `bon-sent-I-littera` — PROLOGUS MAGISTRI, *Cupientes aliquid* | 16–17 | 11102–11283 | to build |
> | `bon-sent-I-capitula` — INCIPIUNT CAPITULA PRIMI LIBRI | 17–21 | 11285–~12190 | to build |
> | `bon-sent-I-prol-comm` — COMMENTARIUS IN PROLOGUM MAGISTRI (divisio + dubia, three parts) | 22–25 | ~12197–12779 | to build |
>
> Landmarks inside the quaestiones, absolute raw lines: `QUAESTIO I.` **9694** · p. 7 head 9823 ·
> p. 8 head 9978 · `SCHOLION` 10044 · `QUAESTIO II.` 10234 · page number `10` 10272 · `11` 10425 ·
> `QUAESTIO III` 10627 · `QUAESTIO IV.` 10907 · `PROLOGUS MAGISTRI` 11102 · `Explicit prologus.`
> ~11283. **Every span still gets fixed positively on the plate; these are search keys, not
> boundaries.**
>
> ## ★★ THE EMPTY-GREP TRAP IS SOLVED — HERE ARE THE ACTUAL SPELLINGS
> The predecessor's greps for `CAPITULA PRIMI LIBRI` / `PROOEMIUM` / `LIBER PRIMUS` returned nothing
> and it correctly refused to conclude the front matter was absent. **It is all there.** The OCR
> prints **`PRO(EMIUM`** (display heading, L8896) · **`PROCEMIUM.` / `PROQEMIUM.` / `PROffilEMIUM.`**
> (running heads) · **`PROCEMII QUAEST. I.`** and **`PROOEMII QUAEST. I.`** ·
> **`aUJlSTIONES PEOiEMII.`** · **`PR0L0GU8 MAGI8TRI`** · **`CAPITULA PRIMl LIBRI.`** — the last with
> a lower-case `l` for `I`, which is why an exact-case grep for `PRIMI` misses four of its five
...
>
> ## ⚠ HAND-OFF INTO `bon-sent-I-proem-q1` — ownership verified, positions NOT
> **p. 6's register runs to n. 12 and nn. 3–12 are q1's** (ten entries; they answer to anchors in
...
> front. **It is not in the way.** Its own frontmatter puts it at raw L13215–13223, and
> `bon-sent-I-d1-divisio` (raw L13224) is `printed_pages: [29]` — so the stub's `DISTINCTIO I.`
> running head stands at the head of **p. 29**, and its `printed_pages: [1] / pdf_pages: [103]` is
> simply wrong. **Not corrected here: pp. 26–29 are not extracted and a page assignment is fixed on
> the plate.** Hand it to whoever extracts that leaf.
>
> ## ★★ THE RAW DROPS A WHOLE APPARATUS ENTRY — COUNT OFF THE PLATE, PER PAGE
> p. 3 n. 8 (`Vers. 25.`) **is absent from the OCR footer block altogether**; the block runs
> `⁶ Vers. 17.` · [7] · [9] · `¹⁰ Psalm. 35, 7`. Counting entries off the raw would have produced
> fifteen where the leaf prints sixteen and shifted every later label against its body anchor.
> **On a Vol I page the OCR is the base for the body; it is not a census of the footer.**
>
> ## ★ ONE PAGE-CROSSING RUNOVER, AND ONE DIGIT CLASS
> p. 2 n. 19 runs over into p. 3, whose footer register **does not open numbered** — rendered joined
> in `[^p2-19]`, and p. 3's body carries **no anchor** at *…sanguis et aqua»*. Six digits were
> corrected on the plate and **all six are the 3/5 class**, not one 1/4 in six leaves: p. 2 n. 1
> `103, 25` · p. 3 n. 3 `Vers. 5` · p. 3 n. 10 `Psalm. 35, 7` · p. 3 n. 15 `distinct. 35` · p. 5 n. 1
> `Vers. 15` · (plus p. 4 n. 4 `Vers. 4`).
>
> ## ⚠ VOL I PLATES: 450 dpi IS NOT WHAT YOU GET, AND THAT IS FINE
> `extract-pages.py --volume vol1 --pages 1-8 --dpi 450` produced **2028 × 2986 px, ~2.2 MB** pages —
> the vol1 PDF's own raster caps it. They are **readable whole, directly, under the API image cap**;
> no `colcrop` was needed for this chunk. Offset `pdf = printed + 102`. pp. 1–8 are extracted.
>
> ## ⛔ THE THREE GUARD-RAIL AUDITS CANNOT SEE A FRONT-MATTER CHUNK
> They select on `bon-sent-{I,II,III,IV}-d(\d+)-`; a distinctio-0 chunk never matches and all three
> print a clean verdict on **0 chunks audited.** Never record such a run as audits-clean. Verify by
...
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
> 2. The About page's **"What Is Known to Be Wrong"** — Wilson's to frame.
> 3. **Book I's remaining six front-matter units**, per the ruling above.
> 4. **Forwarded defect, still nobody's fix:** Quaracchi cite `IV. Sent. d. 2. p. I.` (three chunks
>    do) but the corpus's Book IV d. 2 has no pars chunks. Hand it to whoever works Book IV's body.
>
> ## Corpus state (verified in-session 2026-08-20)
...
> until the proemia close. Queue: **✅ Book II (done) → Book IV (proem ✅, littera + capitula next)
> → Book I's family (8–11 chunks, 8–12 sessions) → Hexaemeron c21–c23 + Scholion → work-close gate
> → deploy.**
>
> ## ✅ `bon-sent-IV-proem` IS TIER 2 — pp. 1–3, 25 apparatus entries (2026-08-20)
> p. 1 nn. 1–8 · p. 2 nn. 1–12 · p. 3 nn. 1–5, zero `[?]` flags. Build **2050 → 2051**;
> `build-citations.py` QA flags **215 → 211** — the chunk contributes **zero** flags of its own and
> **retires four dangling references** (measured by removing the file and re-running).
>
> ## ★★ BOOK IV'S FRONT IS pp. 1–8 AND HAS **THREE** UNITS — FIXED ON THE PLATES, NOT INFERRED
> **The predecessor's "pp. 1–9" was wrong** (it came from subtracting one from
> `bon-sent-IV-d1-p1-littera`'s `printed_pages: [8, 9, 10]`). Measured:
> - **proem pp. 1–3** — closes about a fifth down p. 3 at *Et secundum hoc liber iste⁵ etc.,* under
>   a printer's ornament, the rest of the leaf blank. ✅ done.
> - **littera p. 4** — display heading `LIBER QUARTUS SENTENTIARUM / DE DOCTRINA SIGNORUM`, then
>   Lombard's single full-measure sentence *His tractatis quae ad doctrinam rerum pertinent…*
>   **⚠ Its own apparatus is ZERO** — p. 4's footer nn. 1–7 all answer to the capitula table below
>   it, exactly as `bon-sent-II-littera` came out.
> - **capitula pp. 4–8** — `INCIPIUNT CAPITULA QUARTI LIBRI` opens **on p. 4 under the littera** and
>   the table runs to the head of **p. 8**, where `DISTINCTIO I. / Pars I.` opens on the same leaf.
>   **It is chunked**: apparatus prints ON the table (nn. 1–7 on p. 4, nn. 1–3 on p. 5, …), which is
>   the frozen test. Book IV has **50 distinctions**, so expect a chapter count of the same order as
>   Book II's 265.
> - **⚠ There is NO praelocutio.** p. 1 was extracted and read before anything was written and it
>   opens with the proemium's own display heading. Book IV's gap is Book III's shape, not Book II's.
>
> ## ★★ GUTTERS ARE PER-LEAF HERE AND THE VOLUME DEFAULT IS USELESS IN THIS FRONT
> Measured on the direct ink profile: **p. 1 = 1739 · p. 2 = 1958 · p. 3 = 1729.** CLAUDE.md's vol4
> note gives x≈1880 and `colcrop.py`'s default is 1660; **both truncate a column somewhere in
> pp. 1–3.** ⚠ **p. 1's page image is 3638 px wide where pp. 2–8 are 3719** — the odd/even split
> (≈1730 / ≈1958) is large and real. Profile pp. 4–8 the same way; do not carry 1739 forward.
> ⚠ p. 4 and p. 8 both stack regions (display heading + full-measure littera + two-column table on
> p. 4; table + `DISTINCTIO I.` display heading on p. 8) — **profile the region you are
> transcribing**, per the frozen Vol V lesson.
>
> ## ★ THE RAW IS NOT A BASE FOR THIS FRONT
> `bonaventure_vol4_raw.txt` is column-truncated from ~L946, and the Prolegomena's *Tabula
> collatorum codicum* is interleaved with the opening of the proemium in the same block. The proem
> was set entirely off 450 dpi bands. Expect the same through the capitula. Useful raw landmarks
> only: `LIBER QUARTUS SENTENTIARUM` **L1161** · `INCIPIUNT CAPITULA QUARTI LIBRI` **L1172–1531** ·
> `DISTINCTIO I.` **L1532**. Offset `pdf = printed + 20`; pp. 1–8 are already extracted at 450 dpi.
>
> ## ★ QUARACCHI'S OWN CITATION IS WRONG AT p. 1 n. 2 AND IS TRANSCRIBED AS PRINTED
> *Omne tulit punctum qui miscuit utile dulci* is *Ars poetica* **343**; the leaf prints `v. 243.`
> (settled at 5×). Recorded in the chunk's `## Notes`, not emended. **Do not "fix" it later.**
>
> ## ✅ REGISTER SETTLED AT THE PROEM — reuse, don't re-decide
> *unguentarius* → **"apothecary"** (Douay, Eccli. 38:7) · *pigmentarius* → **"perfumer"** (Douay,
> Exod. 30:35 / Eccli. 49:1) · *pigmenta* → **"confections"** · *unguentum* → **"ointment"** (the
> corpus's existing word) · *unctio* → "unction" as the act, "ointment" only inside the Douay lemma
> *unctiones sanitatis* → "ointments of health" · *thymiama* → "incense" · *suavitas/suavia* →
> "sweetness/sweet" vs *salubria* → "wholesome". ⚠ *pigmentum* is at once spice-confection and
> medicine — **recorded as a loss, not patched with a coinage.**
>
> ## ✅ CONVENTIONS FROM BOOK II STILL IN FORCE
> - **Unnumbered editorial notes are anchored as CLOSING notes** (`[^p<page>-nota]`, end of body,
>   both languages). Frozen in CLAUDE.md.
> - **The frozen capitula test**: apparatus **on the table** ⇒ transmitted text ⇒ chunked.
> - **Marginalia are trimmed from the body and transcribed in `## Notes`** in body order. Book IV's
>   front is dense with them (six on p. 1, eight on p. 2).
>
> ## ⛔ THE THREE GUARD-RAIL AUDITS CANNOT SEE A FRONT-MATTER CHUNK
...
>
> ## ⚠ FORWARDED DEFECT — Book IV's d. 2 pars, still not this front's to fix
...
> running head. Nothing in the proem touches it. **Hand it to whoever works Book IV's body.**
>
> ## ⚠ STILL OWED BEFORE/WITH THE NEXT DEPLOY
> 1. **The `Dictae salutis` fix is still NOT LIVE** (`bon-hex-c8`, p. 372 n. 1).
> 2. The About page's **"What Is Known to Be Wrong"** — Wilson's to frame.
...
>
> ## Corpus state (verified in-session 2026-08-20)
> `build-content.mjs` **2051/2051, 8 books** · `build-citations.py` QA flags **211** ·
> `polish-style-scan` full-corpus: **10 PAIR issues across 5 chunks, all pre-existing** —
> `III-d31-a3-q3`, `III-d32-a1-q2`, `III-d5-a2-q4`, `IV-d14-p2-a2-q1`, `IV-d16-p2-a2-q2` (apparatus
> defs with no matching anchor). None is in this front; none was touched this session.
> ⚠ Nothing pushed; push and deploy are protected.
