# Next session — **d.13 promotion: 5/9 done.** Begin d13-a2-q2.

**d.1–d.12 COMPLETE = 148 chunks. d.13 promotion: 5/9 done** (d13-littera, d13-divisio, d13-a1-q1, d13-a1-q2, d13-a2-q1).
Build: 564 translated, 878 quaestio routes.
...
| `d13-a2-q2`    | 22586–22883 | 4659 | skeleton — **NEXT** |
...
**Promote `d13-a2-q2`** — d.13 Article II, Quaestio II:
*Utrum lux sit forma substantialis, vel accidentalis.* Opens **p.319 L mid**
immediately below d13-a2-q1 scholion item III. Raw lines 22586–22883,
~4659 words — **the longest chunk of d.13**; apparatus-count heuristic
raw=41 — dense, expect ~30+ footnotes. Body likely spans p.319–p.322.

1. Extract + crop p.319–p.322 (p.319 already cached):
   `python3.11 tools/extract-pages.py --volume vol2 --pages 320-322 --dpi 450`
   then `for p in 320 321 322; do python3.11 tools/colcrop.py vol2 $p 1660 3 1.8; done`.
2. Body Latin base from IA djvu OCR raw 22586–22883 reconciled against
   PDF column bands. Watch the column-shattered Respondeo per Vol II
   Override.
3. Apparatus: walk every numbered footer per printed page; large chunks
   often have a scholion + 30+ footers. Consider doing this as a solo
   session per the "large cascade-damaged quaestiones are solo sessions"
   convention.
4. Three audits + smoke build, then two-commit rhythm (chunk +
   content.json; this resume note → **d13-a3-q1**).

After d13-a2-q2: d13-a3-q1 → d13-a3-q2 → d13-dubia. Then d.14 gate
opens (caution: d.14 has multi-pars structure; the chunker created
d14-p1-* and d14-p2-* chunks per the manual-rescue plan).

## Tooling status

- 450 dpi PDF crops cached for pp.274–319 at `/tmp/colcrop/vol2-p*`.
  Generate p.320+ as needed.
