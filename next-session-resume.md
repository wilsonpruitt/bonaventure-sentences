...
> # ★★★ THE ITINERARIUM'S TEXT IS COMPLETE. NINE OF TEN CHUNKS ARE TIER 2.
>
> ## ✅ `prol` · `capitula` · `c1` · `c2` · `c3` · `c4` · `c5` · `c6` · **`c7` (2026-08-14, `37ce1f6`)** — **pp. 295–313, zero `[?]` flags in all nine.** ✅ **Cap. VII closed on the `EXPLICIT ITINERARIUM IN DEUM` colophon, so Bonaventure's text is done.** Remaining: **`bon-itin-scholion` ONLY** — Quaracchi's own work-level scholion, and then the work closes.
> ## ▶▶▶ **NEXT CHUNK = `bon-itin-scholion`** (division 8, `type: scholion`). It opens on **p. 313 beneath a FULL-WIDTH CENTRED `SCHOLION` display heading at ~48 % down**, its two-column body in a **smaller measure**, beginning *Aurei huius opusculi doctrina a viris etiam doctis non raro vel imperfecte, vel omnino perperam intellecta est…*. Index claims pp. 313–316; **verify the end on the plate** — *De reductione*'s half-title is at p. 317/319 and that span is still unverified.
>
> **★★ THE PLATE WORK FOR THE SCHOLION IS ALREADY BANKED — READ `manual-review/itin-scholion-plate-scouting.md` FIRST. Do not re-derive it.** Span, gutters, structure and one digit are settled there; only the transcription remains.
>
> **What the scouting settled (summary only — the file is authoritative):**
> - **Span pp. 313–316**, closing on p. 316 at ~62 % down the right column at *…conferat locos a nobis in notis allegatos.* ✅ **p. 317 IS *De reductione*'s half-title — the standing "317/319" uncertainty is CLOSED at 317.**
> - **★★ THE SCHOLION HAS NO APPARATUS AT ALL.** pp. 314–316 carry **no footer register**; p. 313's six notes are Cap. VII's and belong to `bon-itin-c7`. **`has_apparatus: false`, zero `[^N]` anchors — the first Vol V chunk of which that is true.** It cites inline instead. **No marginalia** on any leaf. The ledger takes a **negative** line; `KNOWN_TOTALS` needs no new entry.
> - **Gutters measured: 313 = 1161** (the Scholion region, *not* Cap. VII's 1166) **· 314 = 1375 · 315 = 1189 · 316 = 1385.**
> - **★★★ A FOURTH MECHANISM BEHIND A NARROW RUN — SCAN SKEW.** pp. 314/315 return 47 px and 53 px bands with 24 px and 13 px "rules", but the leaf is **rotated**: the full-height rule *sweeps* horizontally and its union is subtracted from the band. **Diagnostic: profile the rule's peak x per vertical band and look for MONOTONIC drift** (314: 1384→1368; 315: 1193→1184; p. 316 is the control — 5 px rule, no drift). **A heavily inked rule sits still and darkens; a skewed one walks.** The midpoint is still the right split; the *inference* is what goes wrong.
> - ⚠ **A hand-rolled narrow scan window (x∈1050–1320) misses pp. 314's and 316's gutters entirely** — those sit near 1375–1385 — and returns "no zero column", looking like a failed page. **Use `colcrop`'s own 0.42–0.60 window.** The parity model stays retired.
> - **`supra pag. 47` settled in the 1/4 class**, by finding a known `1` and a known `4` in the same size on the same page (*ibid. pag. 1-47*, three lines below). ★ **The risk ran in the UNUSUAL direction — a true `4` nearly read as `1`.**
>
> **⚠ SIZE: this is the largest chunk in the work by a wide margin — ~4,300 words of Latin in dense small two-column type, roughly DOUBLE a capitulum.** Raw range L55511–56040. Build it with the Péguy incremental-append method as usual; partial progress then survives on disk.
>
...
>
> ## ✅ Verification suite at the `bon-itin-c7` commit: `check-vol5-apparatus.py` **88 chunks / 831 entries** all passed (`KNOWN_TOTALS` fed through p. 313; **no page is PENDING — Bonaventure's text is closed**) · `check-vol5-census.py` rosters agree **88/88**, **seven** page-crossing runovers on the ledger (unchanged — c6 and c7 each checked their joints and found none) · `polish-style-scan --volume 5` CLEAN (88 files) · `build-content.mjs` **2021/2021, 6 books** · `build-citations.py`: c7 contributed **25 records, ZERO dangling**; corpus QA total unmoved at 226. ⚠ **c7's one QA line is a FALSE POSITIVE and a small named class: ordinary Latin *ibidem* in running prose is being read as a citation anaphor** (c7's *ut ibidem a socio eius* in § 3; same at `bon-itin-c4` and `bon-brev-prol-s6`). **No record is emitted, so nothing is fabricated** — it is noise in the unresolved-anaphora list only. The discriminator if it is ever worth fixing: `ibid.` abbreviated is the citation form, `ibidem` spelled in full inside body prose is the adverb. ⚠ QA job 0 (authority-preceded anaphora resolving to fabricated self-references) is UNCHANGED and still open.
> ## ⚠ **Cadence — THE GATE IS NOW ONE CHUNK AWAY.** ONE polish gate for this work (24 pp), at the work close; **deploy boundary = the same point, p. 316.** After `bon-itin-scholion` lands, **both fire together** — the four-pass work-close gate (flag resolution · full-corpus style · boundary sweep over every mid-page and leaf-crossing boundary in pp. 295–316 · disk cleanup) and then the deploy. **The boundary sweep must count leaf crossings BY HAND — `seam-screen.py` is structurally blind to them** (the Breviloquium gate's finding). Nothing is owed before the scholion.
