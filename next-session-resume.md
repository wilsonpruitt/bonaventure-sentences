# Next session — Vol II **DISTINCTION 2 `d2-p1-divisio`** Tier-2 (session 15 — d2-littera DONE)

Updated 2026-05-15 at close of session 14 (`d2-littera` promoted to Tier-2; d.2 now 1/17).

Head commit: `90ec205` — vol2 d.2 promote bon-sent-II-d2-littera to Tier-2 (session 14). Resume commit follows.
...
- **Vol II DISTINCTION 2 RE-CHUNKED (session 13, commit `ee509db`)** — 17 correct skeletons. **1/17 Tier-2**: `d2-littera` complete (session 14, commit `90ec205`). Remaining 16 skeletons; next = `d2-p1-divisio`. See "Next" below.
- Site build: 2 books, 873 chunks, **429 translated** (d2-littera now counted).
...
## Next: DISTINCTION 2 — `d2-p1-divisio` Tier-2 (`d2-littera` DONE session 14)

**`d2-littera` is Tier-2 (session 14, commit `90ec205`).** Re-chunk complete (session 13); all 17 d.2 skeletons have correct boundaries. **Session 15 = promote `bon-sent-II-d2-p1-divisio` to Tier-2** (raw 4418–4462; the `COMMENTARIUS IN DISTINCTIONEM II. Pars I.` — DIVISIO TEXTUS + TRACTATIO QUAESTIONUM, begins full-width mid-p.54 right after Cap. VI). Then quaestiones ~1/session. Pre-rechunk backup: `_backup-d2-pre-rechunk-20260515/` (gitignored).

**`d2-p1-divisio` starting points (mapped while reading p.54 for the littera, session 14):**
- Body begins **mid-p.54**, full-width below the two-column littera: `COMMENTARIUS IN DISTINCTIONEM II.` / `Pars I.` / running title `Quando creati sunt Angeli et ubi` / `De angelica [natura]` / `DIVISIO TEXTUS` → *"Supra egit Magister de rerum productione in generali. In hac parte agit de productione rerum in speciali. Et quoniam triplex est differentia creaturae, scilicet spiritualis et corporalis et composita ex utraque, ideo haec pars habet tres…"* → `TRACTATIO QUAESTIONUM` → *"Ad intelligentiam huius partis, in qua quaeritur de mensura conditionis angelicae naturae, duo principaliter quaeruntur. Primo de mensura eius in se. Secundo per comparationem ad mensuram esse corporalis naturae. … Circa primum tria quaeruntur. Primo, utrum spiritualia habeant mensuram propriam. Secundo, utrum habeant mensuram unicam. Tertio, utrum habeant mensuram simplicem aut compositam, sive stantem vel successivam."* (continues p.55).
- **p.54 cross-chunk footer for the divisio:** the **`NOTAE AD COMMENTARIUM`** sub-block on the p.54 footer (after the littera's `Cap. 14, 13.` note 3) is the divisio's apparatus head — first note read session 14: `¹ In Vat. additur ex auctoritatibus.` The 3 littera notes above it (`Vat. sola ordinationis.`; `De Gen. ad lit. … Confess. c. 9.`; `Cap. 14, 13.`) are already in `d2-littera` [^14]–[^16]; do NOT re-take them.
- Crops cached 450 dpi: `/tmp/colcrop/vol2-p05{4,5}-{L,R}-{0,1,2}.png`, `raw/vision/vol2/p-05{4,5}.png`. p.55 not yet read.
...
### `d2-littera` — DONE (session 14, commit `90ec205`). Record + correction:

Structure (Cap. I–VI, pp. 52–54), full crosswalk, and the cross-chunk footer resolution are documented in the chunk's own `## Notes` (`vol2/bon-sent-II-d2-littera.md`). Key correction for future reference:
