...
>
> ## ⬜ **NEXT: `bon-hex-c2` (Collatio II), which opens on p. 336.** Extract its plates per collatio (never in bulk); its span ends at the next `COLLATIO III.` heading, to be fixed on the plate. The frozen conventions in repo CLAUDE.md § HEXAEMERON now have a worked example to follow in `vol5/bon-hex-c1.md`.
>
