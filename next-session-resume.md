# Next session — **d24-p1-a2-q2 DONE 2026-05-29 → next action: dispatch `bon-sent-II-d24-p1-a2-q3` (ARTICULUS II QUAESTIO III *Utrum divisio voluntatis per naturalem et deliberativam sit per diversas potentias* [= *per deliberativum et naturale*]; skeleton, opens raw 39610 QUAESTIO III on p. 565). Hand-off RECEIVED from a2-q2: NONE — q2's SCHOLION (I–III) + all p.563–565 footers are self-contained; QUAESTIO III opens fresh on p.565 (raw 39610) with its own footer sequence. Body opener begins after the q3 title (raw 39612+). Apparatus-count audit shows raw=19 footers for a2-q3. Extract pp.565-? crops as needed (`/tmp/colcrop/vol2-p565-*` cached for the q2 scholion + start of q3).**
> **ARTICULUS II = *De divisione potentiarum animae* (4 questions, all in Pars I); TRACTATIO (raw 39105–39130, verbatim): Primo *per cognitivam et affectivam, sive per rationem et voluntatem* (= a2-q1, DONE); Secundo *per portionem superiorem et inferiorem* (= a2-q2, DONE); Tertio *per deliberativum et naturale* (= a2-q3, NEXT — OCR title *Utrum divisio voluntatis per naturalem et deliberativam sit per diversas potentias*, order flipped from TRACTATIO); Quarto *per possibilem et agentem* (= a2-q4). NOTE: Pars II numbering (p2-a*, p2-dubia) is auto-chunk-suspect — re-chunk-verify before promoting Pars II. Stray dups still pending cleanup: `d24-p1-dubia-dup2` (apparatus audit flags it — git rm when its real chunk is promoted).**
