# Next session — **`d31-dubia` DONE 2026-05-30 (Tier-2, commit `5a23dc4`). DISTINCTIO XXXI now FULLY Tier-2** (littera + divisio + a1-q1/q2/q3 + a2-q1/q2/q3 + dubia = 9 units; d.30 gap [octo-nominibus dubia] also closed this session). **NEXT ACTION = dispatch `bon-sent-II-d32-littera`** (Lombard's text of DISTINCTIO XXXII, opens raw **52747** / printed p.756, offset pdf=printed+22; the running head digit `737` is OCR-mangled — trust the +22 offset and running-head text). **d.32 littera range:** raw **52747** (`DISTINCTIO XXXII.` + `Cap. I.` *Quomodo originale peccatum dimittitur in baptismo*) → immediately before **`COMMENTARIUS IN DISTINCTIONEM XXXII.`** (raw **≈52902**). The littera is Lombard's chapters **Cap. I–VIII** (Cap. I = baptism remits originale; Cap. II–VIII follow at raw 52770/52821/52833/52862/52883/… ); first dubium-marginal "Dubium I." sits inside Cap. I. **d.32 commentary structure after the littera:** `COMMENTARIUS IN DISTINCTIONEM XXXII.` (raw ≈52902) → `DIVISIO TEXTUS.` (raw ≈52906) → `TRACTATIO QUAESTIONUM.` (raw ≈52929 — *quaestio circa tria*; Circa primum quaeruntur duo: Primo utrum baptismus deleat originale…) → `ARTICULUS I.` `QUAESTIO I.` (raw ≈52939) → later `ARTICULUS II.` `QUAESTIO I.` (raw ≈53313). Build the littera as its own big Tier-2 chunk (template = `vol2/bon-sent-II-d31-littera.md` / d8-littera), then the divisio chunk holds COMMENTARIUS + DIVISIO TEXTUS + TRACTATIO QUAESTIONUM. **No hand-off into d32-littera** — DISTINCTIO XXXII opens fresh; d31-dubia FORWARDED nothing (all p.756 footers anchored within the dubia). **No decade polish until d.40.**
