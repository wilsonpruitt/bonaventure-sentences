# Next session — Vol II **DISTINCTION 2 `d2-p1-a1-q1`** Tier-2 (session 16 — littera + divisio DONE)

Updated 2026-05-15 at close of session 15 (`d2-p1-divisio` promoted to Tier-2; d.2 now 2/17).

Head commit: `5b47955` — vol2 d.2 promote bon-sent-II-d2-p1-divisio to Tier-2 (session 15). Resume commit follows.
...
- **Vol II DISTINCTION 2 RE-CHUNKED (session 13, commit `ee509db`)** — 17 correct skeletons. **2/17 Tier-2**: `d2-littera` (session 14, `90ec205`) + `d2-p1-divisio` (session 15, `5b47955`). Remaining 15 skeletons; next = `d2-p1-a1-q1` (first quaestio). See "Next" below.
- Site build: 2 books, 873 chunks, **430 translated** (littera + divisio counted).
...
## Next: DISTINCTION 2 — `d2-p1-a1-q1` Tier-2 (littera + divisio DONE)

**`d2-littera` (session 14, `90ec205`) and `d2-p1-divisio` (session 15, `5b47955`) are Tier-2.** **Session 16 = promote `bon-sent-II-d2-p1-a1-q1` to Tier-2** — the first quaestio, *Utrum spiritualia habeant mensuram propriam* (ART. I, QUAEST. I), raw **4463–4696** (the largest p1 quaestio, ~234 raw lines). Then a1-q2, a1-q3, a2-q1… at ~1/session. Pre-rechunk backup: `_backup-d2-pre-rechunk-20260515/` (gitignored).

**`d2-p1-a1-q1` starting points (mapped while reading p.54 for littera/divisio):**
- Opens on **printed p.55** at the running head `DIST. II. P. I. ART. I. QUAEST. 1.` (raw ~4462–4463 boundary; p.55 = PDF p.77). The ARTICULUS I opener (`Circa primum tria quaeruntur. Primo, utrum spiritualia habeant mensuram propriam…`) is the *tail of `d2-p1-divisio`'s TRACTATIO QUAESTIONUM* (already Tier-2) — a1-q1 begins at the QUAESTIO I body proper; any ART. I sub-divisio that precedes the *Videtur* args folds into this chunk (Vol II convention).
- **Full cascade-quaestio discipline applies** (Vol II override): `python3.11 tools/extract-pages.py --volume vol2 --pages <printed> --dpi 450` then `python3.11 tools/colcrop.py vol2 <printed>`; read L col then R col, body bands then footer; reflow column-by-column. p.55 **not yet read** — extract/crop p.55 (and following pages until QUAEST. II's running head) fresh. Footnote numbering restarts per printed page; build the per-page→chunk crosswalk and watch the q1↔q2 cross-chunk footer split.
- Apparatus-count heuristic flags `d2-p1-a1-q1` raw=40 (highest in d.2) — that is two-column cascade-footer noise across its multi-page span, not a true 40-note count; ground-truth is the printed footers walked page by page.
- Crops cached so far: only pp. 52–54 (`/tmp/colcrop/vol2-p05{2,3,4}-…`). p.55+ need a fresh extract.
