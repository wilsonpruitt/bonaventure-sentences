...
def main() -> int:
    chunks: list[Path] = []
    # vol5 holds WORK chunks (`bon-brev-*`, later `bon-itin-*` etc.), which have
    # no distinctio number and so do not match the Sentences id prefix. Added
    # 2026-07-28 — before that this audit silently skipped Vol V entirely, the
    # same blindness that hid vol3/vol4 from polish-style-scan.py for four
    # volumes. The per-chunk checks below are id-agnostic; only the legacy
    # duplicate scan is Sentences-shaped, and it no-ops on unmatched names.
    ...
    results = []
    tier2_count = {"vol1": 0, "vol2": 0, "vol3": 0, "vol4": 0, "vol5": 0}
    skeleton_count = 0
    for p in chunks:
        r = audit_chunk(p)
        results.append(r)
        if r["tier2"]:
            ...
            key = next((v for v in ("vol1", "vol2", "vol3", "vol4", "vol5")
    ...
    # Build report
    lines: list[str] = []
    lines.append("# Pass 2 — Style/Formatting Audit (full corpus, Vols I–V)")
    lines.append("")
    lines.append(
