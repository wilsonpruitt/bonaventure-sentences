...
5. **Refused — NOT fixed, flagged for a future full-paragraph rebuild (out of this session's scope).** While verifying item 2 against print, the printed page 759 text revealed that the chunk's existing Latin/English wording immediately preceding "ergo iste potest videre[^24][^25]" is **not a verbatim match** to the print at that clause: the actual printed sentence reads "...hoc dictum¹ esse verum, est possibile, quia possibile accipitur ibi respectu veri; et dictum esse possibile, quando dicit ordinem ad esse vel ad esse verum, dicit potentiam communiter ad activam et passivam, superiorem et inferiorem. Sed tunc non sequitur de re: ergo iste potest videre; quia² dicit potentiam activam creatam..." — i.e. the true printed superscript `¹` (= this chunk's `[^24]`, per its def discussing the Vat. variant "adiungit *iste potest videre*") sits on "dictum¹" in a clause that is not currently transcribed in the chunk at all, and printed `²` (= `[^25]`) sits on "quia²" rather than being stacked with `[^24]` on "videre". The chunk's own already-existing Latin/English wording in this clause appears to be a paraphrase/compression that predates this session (not something this pass introduced). **Per task instructions ("never change Latin wording, English wording... only fix/add/remove `[^N]` marker tokens"), this was left untouched** — correcting it would require adding the missing "hoc dictum..." clause and repositioning `[^24]`/`[^25]` independently, which is a wording-level Tier-2 rebuild, not a marker-only fix. `[^24]` and `[^25]` remain, each used exactly once, content-plausible for the immediate vicinity (both p.759 footer entries 1–2 discuss variants clustered around "iste potest videre"/"activam"/"poterit" in this same sentence), so they are not flagged as broken anchors by the health-scan — but the underlying prose imprecision is recorded here for a future full re-verification pass against p.759.
