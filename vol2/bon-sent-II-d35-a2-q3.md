...
- **p.835**: Solutio ad-1.2.3 + ad-4 + ad-5 + SCHOLION I (begins p.835 R col bottom). Footers 1–6 → [^19]–[^23] (six footers, five body markers — Solutio carries five): [^19] ad-1.2.3 *eius pars aliquota¹* (Vat. addit culpa) + [^20] *Dionysius… / aliquota* (Id est quae aliquoties replicata) — see [?] note; [^21] ad-4 *dicit Augustinus⁴* (Serm. 151); [^22] ad-5 *in ea⁵ aufertur radix* (Edd. in eo); [^23] ad-5 *ad bonum⁶* (Vide supra d.7 p.1 a.1 q.1).
...
**[?] flags.**
- **[?] p.835 footers 1–2 vs body markers ([^19]–[^20]):** p.835 prints six footers but the Solutio ad-1.2.3 region shows three close superscripts (`tanto peccato`/`pars aliquota`/`Dionysius`). [^19] (`Vat. addit culpa`) is placed at `eius pars aliquota` and [^20] (`Id est, quae aliquoties replicata mensurat totum` — a gloss on *pars aliquota*) is placed at `dicit Dionysius`. The Quaracchi superscripts in this band are OCR-mangled (`'`, `°`, `'`) and the 450 dpi crop is faint at the column gutter; the two notes plausibly belong to `tanto peccato` (Vat. addit *culpa*) and `pars aliquota` (the *aliquoties replicata* gloss). The anchor *positions* of [^19]/[^20] are therefore approximate within the ad-1.2.3 sentence; the apparatus *content* and count are exact. RESOLVE at the d.31–d.40 polish-blocker with a 600 dpi pass.
- **[?] [^22] (Serm. 151) tail truncation:** the footer ends `…Contra Iulian. Pelagian. c. 16. n.` — the section number `n.` is cut at the foot of p.835 in both OCR and 450 dpi crop (runs off the plate). Rendered as `n. [—]`. ACCEPT-ILLEGIBLE pending 600 dpi; low value (a secondary cross-ref number).
