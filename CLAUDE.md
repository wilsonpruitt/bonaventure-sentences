...
| 4 | QD de perfectione evangelica | **117–198** | `perfectione-evangelica` | 10 | **COMPLETE 2026-09-17 — all TEN chunks Tier 2, pp. 117–198, zero `[?]` in every one; **WORK-CLOSE GATE RUN AND CLOSED 2026-09-17, zero text defects** (`manual-review/vol5-perfectione-evangelica-workclose-gate.md`); the front is PUSH then DEPLOY, each Wilson's separate OK. Mini-pilot run 2026-09-15** (§ PERFECTIONE EVANGELICA below; evidence `manual-review/perfectione-evangelica-pilot-scouting.md`). No half-title (confirmed); body 117–198, ends on p. 198 with NO colophon, p. 199 = Breviloquium half-title. **10 chunks, one per articulus** (Q. I undivided). |
...
| 11 | Sermones selecti | **533–579** | `sermones-selecti` | 14 | **✅ COMPLETE 2026-09-19 — all FIVE chunks Tier 2; WORK-CLOSE GATE RUN AND CLOSED, zero text defects (`manual-review/vol5-sermones-selecti-workclose-gate.md`). THIS CLOSES VOL V. Front = PUSH then DEPLOY, each Wilson's separate OK.** Mini-pilot run 2026-09-18 (§ SERMONES SELECTI below; evidence `manual-review/sermones-selecti-pilot-scouting.md`). 533 half-title, 534 blank, **body 535–579 (45 pp), ends on `EXPLICIUNT SERMONES SELECTI.`**, 580 blank, 581 index. ⛔ **FOUR sermons + ONE annexed *Tractatus de plantatione Paradisi* = FIVE chunks** — not "dozens." ✅ **All five pilot decisions RULED (Wilson, 2026-09-18); the front is `bon-serm-s1` (pp. 535–538).** |
...
### MYSTERIO TRINITATIS — mini-pilot conventions (frozen 2026-09-05; evidence in `manual-review/mysterio-trinitatis-pilot-scouting.md`)

*Quaestiones disputatae de mysterio Trinitatis*, work 3, book id 9, slug
`mysterio-trinitatis`, id prefix **`bon-qmt-`**. **The second *quaestio disputata*.** English
title: **"Disputed Questions on the Mystery of the Trinity."**
**NO half-title leaf** (p. 1's covers all three QD — the work-2 prediction, confirmed) ·
**p. 44 MEASURED BLANK (0.0007 %)** · **body opens p. 45** on a `QUAESTIONES DISPUTATAE /
DE MYSTERIO TRINITATIS` display heading + ornamental rule · **work ends p. 115, NO colophon**
(the genre's no-`EXPLICIUNT` close, as predicted) · **p. 116 blank** — ⚠ it measures **0.1333 %**,
far above p. 44's, and **the excess is the scan's dark gutter edge, identified by READING the
plate**; a blankness number is a screen, the plate is the evidence.
**Body extent pp. 45–115 = 71 printed pages.** Raw band **L16037 → L26475** (~147 lines/page).

- **FIFTEEN CHUNKS, ONE PER ARTICULUS: `bon-qmt-q{1..7}-a{1,2}` + `bon-qmt-q8`**,
  `division: <quaestio 1–8>`, `articulus: <1|2>`, `type: quaestio`. **Eight quaestiones,
  fourteen articuli, Q. VIII undivided** — volume index and a header grep over the band agree
  exactly. ⭐⭐ **The unit was settled by QUARACCHI'S OWN INTERNAL CROSS-REFERENCES: of 27
  `supra`/`infra q. N.` references inside the work, 26 carry `a. N.`** (the single bare one is
  `infra q. 7.`), plus two intra-question `supra a. N`. **Splitting on the quaestio would break
  the citation match at 26 sites in this work alone.** ★ The frozen anchor test was still run
  and returned nothing (below) — which is what freed the decision to rest on the citations, a
  *stronger* warrant than an anchor, not a weaker one.
...
- **▶ THE WORK-LEVEL PROEMIUM FOLDS INTO `bon-qmt-q1-a1`.** p. 45 carries seven lines between
  the display heading and `QUAESTIO I.` (*Volentes circa mysterium Trinitatis aliquid indagare…*
  plus the two *praeambula*, which are q. 1's two articulus subtitles). **Unanchored.** It folds
  under the frozen short-opener rule; a `division: 0` prologus chunk was **rejected** — the
...
- **Index-derived spans, EVERY END PROVISIONAL** (an index span's last page is the next unit's
  ...
  q7-a1 **106–109** *(measured **106–109**, shared both ends)* · q7-a2 **109–111** *(measured **109–112**, shared both ends)* · q8 **112–115** *(measured **112–115**; end fixed from p. 117's heading)*. Headers at raw L16054/16058 · 17009 ·
  ...
  the leaf's FIRST WORD), nn. 5–6 forward to `q3-a2`, and n. 2 runs over the gutter. ★ **The span end
  was fixed on the plate BEFORE the plates were pulled, per the order `q2-a2` left, and the index was
  right this time — being right is not the same as being known to be right.**
  ...
  subtitle and the two-column opener stand on p. 78 and belong to `q4-a1`; **all four of that leaf's
  register entries stay with `q3-a2`**, because the opener is unanchored and all four notes pair with
  replies 10, 13, 14 and 16. ⚠ **The index shape here was the dangerous one** — `q3-a2 73–78` against
  `q4-a1 79–84`, non-overlapping, which is exactly the shape that was WRONG at `q2-a2` — and it is
  right only because p. 78 was extracted alone and read before the rest of the span was pulled.
  ⭐⭐ **BOUNDARY 7 (p. 84) MEASURED: it FORWARDS, split 1/6.** Art. I closes **five lines into the
  right column**; `ARTICULUS II.` then stands full measure and Art. II fills the leaf. Only **n. 1**
...
- **⚠⚠ THE SCAN CLIPS THE GUTTER-SIDE MARGINALIA ON SOME LEAVES (found `q3-a1`, 2026-09-06; ⛔ SCOPED
  ...
  not of the work. Check it on every leaf; the failure mode is silent).** Quaracchi sets a marginal gloss outside each column: the
  wide outer margin for one, the narrow gutter-side margin for the other. **The narrow one is cut by the
  edge of the scanned image.** On rectos it costs one letter (`[A]d oppositum.` · `[A]pplicatio.` ·
  `[C]orollarium.`); on p. 70 **eight right-hand glosses lose their line-ends** and three lose more than
  a letter. ⭐ **The `bon-brev-p7-c7` test governs — restore only what is determinate from matter printed
  on the same page** — and it must be applied strictly: on p. 70 six glosses were restored (each either
  a standing form this work uses elsewhere, or a word-for-word summary of the sentence it stands against)
  and **two were deliberately left incomplete**, because the body beside them makes a guess easy and
  that is precisely the reason to refuse it. ⚠⚠ **CROP TO THE FULL IMAGE WIDTH when you check.** A crop
  that stops even 40 px short of the edge produces the identical symptom and will make you record a
  clipping that is your own.
- **⚠⚠ A THIRD DEFECT CLASS: AN INK SMEAR — ink ADDED, not lost (found `q3-a1` p. 71, 2026-09-06).**
  The shakedown gate's two-part rule covers *missing* letters. **A smear is neither branch.** A heavy
  stroke lay across two lines of p. 71's left register, and the raw returned garbage at exactly those
  two places. **The type is present under the ink, not absent from the page**, so **nothing is bracketed
  and nothing is reproduced as a gap** — the letters are read through the ink where they can be, and
  where they cannot, **the reading is taken from the target the note cites, not from the type.**
  ⭐ The gate's diagnostic still works, read the other way: **the neighbours are DARKER than their line,
  not lighter.** ⭐⭐ **And a digit no eye can read is still recoverable:** p. 71 n. 1's `d. 27. p. I. q. 3`
  had its final digit blotted out entirely, and `q. 3` was fixed because it is **the only one of d. 27
  p. I's four questions that contains *quo est* at all** (five hits against zero, zero, zero); the same
  note's `d. 23` overturned the raw's `d. 33` because **I. Sent. d. 23. a. 1. q. 3** is the
  *essentia/subsistentia* question and carries *quo est* nine times.
- **⭐ A FOURTH ATTESTATION OF THE COMPOSITOR'S-BLANK CLASS, AND THE FIRST WITH TWO IN ONE PARAGRAPH
  (`q3-a2` p. 78, 2026-09-06).** Reply 16 prints *…sed relati〈blank〉Unde dici potest, vel quod ratio
  illa non con〈blank〉aut quod propositio illa…*, where the sense plainly wants *relativa.* and
  *concludit,*. **Neither is supplied.** ⭐ **The ratified rule was applied by MEASUREMENT, not by
  eye**, and that is the transferable part: the letters flanking each blank run to a minimum grey of
  **13–36** with the same dark fraction as their line's opening words — full strength on both sides,
  so the first branch governs and the gap is reproduced. Three checks ruled out the alternatives:
  the lines above and below run unbroken through the same pixel column (not a stain, a paper repair
  or a scanner artefact); the paper inside the blanks measures **min 141 and min 228** against a text
  minimum of 13 (no ghost of a failed impression); and the raw shows the same two gaps. English
  carries `[*sic*]` at each, on the p. 59 *eandem* precedent.
- **⛔ A RAW MARKER GLYPH IS NOT EVIDENCE OF AN ANCHOR (`q3-a2` p. 77, 2026-09-06).** The raw prints a
  marker after *…a sua proprietate personali* and reply 4 in fact carries none; trusting it pushed
  every later anchor on the leaf down one and produced a **seventh anchor on a six-note leaf**, which
  `check-vol5-apparatus.py` refused as *anchored but undefined*. ⭐⭐ **The correction was then
  confirmed from the TARGETS rather than from the type**, and this is the cleanest such confirmation
  the work has produced: `I. Sent. d. 19. p. II` has four questions dividing the ground exactly —
  **q. 2 *Utrum sit ponere totum universale*** (n. 1 → reply 3, on *commune*/*universale*), **q. 1
  *Utrum in divinis sit ponere totum integrale*** (n. 3 → reply 6, on part and whole) — and n. 3's own
  closing *Ibid. q. 4. insinuatur solutio seq.* points at **q. 4 *Utrum in divinis sit differentia
  secundum numerum*, which is reply 7, the very next reply.** ⚠ Its companion, from the same chunk's
  p. 74 n. 2: the raw's `Ethic:` and a 3× plate read agreeing are **one witness read twice**, not two
  — at 22× the mark is a baseline point plus a wedge-shaped blot, and the note reads `Ethic.`
- **⛔⛔ A RUNOVER LEDGER LINE READING `-` IS AN ASSERTION NOTHING RE-DERIVES (`q4-a1` on `q3-a2`, 2026-09-06).**
  `bon-qmt-q3-a2` recorded p. 78 as forwarding no runover and stated it positively — *zero runovers,
  neither gutter-crossing nor page-crossing*. **p. 78 n. 4 in fact runs over onto p. 79**, where the
  footer opens with an unnumbered, **flush-left** paragraph before n. 1 (*Distinctio opponitur
  identitati…*), continuing n. 4's own subject and present in the raw as well. Repaired in place; the
  ledger line went from `-` to `p.78 n.4:page`. ⛔ **`check-vol5-census.py` checks that every chunk
  APPEARS in the ledger, never that a negative is true**, so a false negative survives every audit.
  ▶ **THE ONLY THING THAT CATCHES ONE IS THE NEXT CHUNK READING THE INCOMING LEAF'S REGISTER FROM THE
  TOP, BEFORE ITS FIRST NUMERAL. Do it on every leaf.** The typographic signature is decisive: a
  runover's first line is set **flush left**, where every numbered entry is indented under a superscript.
- **⭐⭐ WHEN A HIGH-ZOOM PLATE READ RETURNS A NON-WORD, MEASURE THE INK BEFORE BELIEVING IT
  (`q4-a1` p. 79 n. 7, 2026-09-06). Corpus-general.** The note ends *Augustini* **sementia** at 12×,
  and the raw independently gave *seinentia*; neither is a Latin word. **Measurement settled it**: the
  word runs to a minimum grey of **58** with dark fraction **0.040**, against **6 / 0.109** for
  *Augustini* immediately before it and **2 / 0.106** for *nota 5.* on the same line — the ratified
  rule's **second branch**, *the type was set and this copy failed to take it*. A contrast-stretched
  read at 14× then shows a **t whose stem and ascender are present and whose crossbar is not**: the
  word is ***sententia***. ⭐ **Transcribed plain, NOT bracketed** — the precedent restorations
  (*rebu[s]*, *plura[lit]as*) bracket letters that are *absent*, and here the letter is present minus
  one stroke; the disclosure goes in the chunk's Notes instead. ⛔ **A dropped crossbar turns `nt` into
  `m` and hands you a plausible string no dictionary contains.**
- **⚠ THE DIGIT-MULTISET CHECK HAS A FALSE-POSITIVE CLASS: SPELLED-OUT CHAPTER NUMBERS (`q4-a1`, 2026-09-06).**
  Quaracchi writes *tertio Regum octavo*, *Sapientiae undecimo*, *Isaiae quadragesimo*; the house form
  set at `q1-a2` (*Genesis, chapter 18*) renders these with numerals, so the English section carries
  digits the Latin does not and the multiset disagrees **for a reason that has nothing to do with page
  markers**. ▶ **Compare the `<!-- page N -->` markers DIRECTLY — same list, same order, each at a word
  boundary in both languages. The multiset is a SCREEN, not the test.**
- **⚠ THE MARGINALIA-CLIPPING CHECK IS SYMMETRICAL, AND THE FALSE POSITIVE IS AS EASY AS THE FALSE
  NEGATIVE (`q4-a1`, 2026-09-06).** At ⅓ scale p. 79's `Fundamenta.` appeared to have its `F` cut by
  the image edge, and a column-crop read agreed; **at 9× on a full-image-width strip the `F` is intact
  with ~60 px of clean paper before the scan's dark gutter band.** A dark edge band beside a gloss
  reads as a clipped letter at low magnification. ▶ **Only a full-width strip read at high
  magnification decides it, in either direction.**
- **⚠ FIVE OF SIX PAGE BREAKS ON A SPAN CAN FALL MID-WORD.** Set the `<!-- page N -->` marker **at the
  word boundary immediately before the split word**, so the word is carried whole into the new page, and
  mark **both languages at the same place**. Splitting a word across the marker parses and passes every
  audit, and leaves a fragment either side of a comment that a later reader cannot tell from a typo.
  ★ The cheap check that catches a mismatch: **the digit multiset of the Latin section and of the English
  section must be identical** once `[^pNN-N]` labels are stripped — a page marker present in one language
  and not the other shows up there immediately.
- **⛔ p. 51 n. 1 PRINTS WITHOUT ITS NUMERAL, and the plate and the raw agree.** The register's
  ...
  **The anchor exists** — p. 51's opening sentence ends *…consideranti apparet*¹ (raw L16909).
  ⚠ **The leaf carries six entries, nn. 1–6; do not renumber the register, and do not assume an
  unnumbered entry is a runover from the previous page** — that hypothesis was tried here and
  killed by finding the body anchor. This is the inverse of the digit class the last two works
  fought (glyph misread) and it is caught the same way: **pair every footer entry with a body
  anchor, never read the register alone.**
- **⚠ THE RUNNING HEAD NAMES THE UNIT THAT *OPENS* ON THE LEAF, NOT THE ONE THAT FILLS IT.**
  p. 51's reads `…QUAEST. I. ART. II.` over three-quarters of a leaf of **Art. I**. Verso heads
  are the flat `QUAESTIONES DISPUTATAE`. **Never derive ownership from a running head** — and it
  matters more here than anywhere, since `ART. N` appears nowhere else outside the headings.
- **Page map: 49 surviving numerals over 71 leaves, strictly monotonic, ZERO disagreements** — no
  repeat of work 2's p. 35 → `33`. ⚠ **It is a PREDICTION table**; every chunk fixes its own
  pages on the plate. Full list in the scouting doc.
- **Apparatus density:** p. 45 = **6** (n. 4 runs over the gutter), p. 51 = **6**. ~7/page × 71 pp
  ≈ **500 for the work** — a PLANNING figure only, of the kind work 2's p. 43 n. 3 punished.
  ⭐ p. 45 n. 1 is *Similis quaestio solvitur I. Sent. d. 8. p. I. a. 1. q. 2.* — **an outbound
  citation into our own deployed Vol I on the work's first leaf.**
- **Gutter, p. 45: ADOPT 1159.** `colcrop.py vol5 45` gives **1129 on a 1 px run** — the
  work-opening failure at its purest (display heading, proemium and two heading stacks all cross
  the gutter). Body rows 0.53–0.82 give six windows at **1159–1160 on 62–65 px runs** against six
  at 1133/12 px; band **1128–1189 (62 px)**, centre-rule island **1155–1161** (peak 614).
  ⚠ Work 2's p. 3 adopted **1130** on a leaf of the same kind — 29 px away. **Measure every leaf.**
- **REGISTER — measured at the pilot, so no chunk has to re-ask the census:**
  1. ***intellect-* occurs 96 times and `intellectus agens`/`possibilis` occur ZERO times.**
     ⭐⭐ **The ratified narrow exception is UNREACHABLE in this work**, so all 96 go to
     **understanding** and **any bare English "intellect" in a built chunk is a defect by
     construction** — work 2's mechanical control, with a cleaner denominator.
  2. ***lux* 26 / *lumen* 15 — ruling 2 is LIVE**; the translator's note is needed again.
     ⚠ **The Ps. 35:10 exemption is UNREACHED**: *In lumine tuo videbimus lumen* does not occur.
     The psalm IS quoted — p. 115's last note gives Ps. 35:10 *Apud te est fons vitae* — **a
     different clause carrying no *lumen*. The exemption is scoped to the WORDING, not the verse
     number.**
  3. **`contuit-` occurs ZERO times — ruling 3 is absent, not broken.**
  4. ⭐⭐⭐ ***vacatio*/*quies* IS FINALLY TESTABLE, and the decisive site is known.** Open since
     `bon-praec-c4` and closed as *untestable* on work 2, the question meets both families here:
     the noun *quietis* twice in **`q5-a2`** (raw L23636, L23645) and *vacando*/*vacabimus* in
     **`q7-a2`** (raw L25824, L25900) — **and L25824 holds both in one clause**: *non **quiescit**
     nisi in summe amabili **vacando***. ⛔⛔ **CORRECTED 2026-09-07 BY `q5-a2` ON THE PLATE: L23636 IS
     NOT THE NOUN.** It is *…silentium **quietis-***, the first half of ***quietissimum*** broken across
     a line by Quaracchi's own hyphenation. **A stem census over two-column OCR counts line-broken
     half-words as hits, and the failure is silent because the fragment is a real Latin string** — the
     same class as the near-miss below, caught by the opposite error. `q5-a2` therefore holds **ONE**
     noun site, **L23645** (*in libertatem supercaelestis quietis plenius elevetur*), **ruled → "rest"**,
     with *quietissimum* kept adjectival → "most quiet". ▶ **That settles the *quies* side and NOT the
     pair; the families meet only at `q7-a2` L25824, which is now the sole deciding site.**
     ⚠ **DO NOT SETTLE IT AT THE PILOT.** It is settled in
     `q5-a2` and `q7-a2` with the plate in view; **the pilot's job was to establish that it CAN be
     settled and where**, so the chunk that meets it knows it is the deciding chunk. **A pilot's
     term census is a MAP, not a ruling.**
     ⚠⚠ **The near-miss, worth more than the finding:** the first census ran
     `vacatio|vacare|vacat` and returned **zero** — it misses *vacando* and *vacabimus*, which are
     the only two sites in the work, and the question would have closed "untestable" a third time
     ...
     wrong.**
     ⛔⛔ **MET AT `q7-a2` (2026-09-15) — AND THE SITE IS TEXTUALLY INSECURE.** p. 111 prints the clause as banked, but
     its note n. 1 argues about the word: *D potius legi deberet **vetando**. Fortasse originale habuit **amabili et
     amativo**…* — the one witness reads rather *vetando*, and the editors suspect the original lacked the gerund. The
...
- **Marginalia DENSE, one per argument or reply** (p. 45: `Triplex via.`, `Via prima.`,
  ...
  body, transcribed to the `## Notes` Marginalia list in body order, per the standing convention.
- **Registry:** add `mysterio-trinitatis` to `WORKS` (book 9, tome 5, `divisionLabel:
  "Quaestiones"`) **and a NEW branch in `buildWorkChunkTitle`, because this is the first Vol V
  work whose chunk is finer than its division** — return **`Quaest. N, Art. M`** when
  `meta.articulus` is present and **`Quaest. N`** when it is not (Q. VIII). Leave the
...
- **Cadence: TWO gates.** **Shakedown at the close of q. 2 (p. 68 — measured, NOT p. 67; four chunks / 24 printed pp.)**
  ...
  not update the site — run `python3.11 tools/build-index-json.py` after any ledger rebuild,
...
- **✅ `bon-qmt-q4-a1` IS BUILT (pp. 78–84, 44 entries, zero `[?]`, 2026-09-06) — QUAESTIO IV IS OPENED.**
  Thirteen arguments that the divine *esse* is most infinite (marginale `Fundamenta.`), a **`Contra`
  series of fifteen** (marginale `Ad oppositum.`), `CONCLUSIO` in-column, a *Respondeo* whose whole
  work is a **double distinction** (*infinitum privative* / *negative*; *finis-terminus* /
  *finis-complementum*; under the first terminus *quantitas molis* / *quantitas virtutis*), and
  **fifteen replies**, which answer the **`Contra`** series — checked from the replies' own quoted
  lemmata, since `q3-a2`'s answered the *opening* series. Registry **division 4** went in with it,
  subtitle *Utrum trinitas stet cum summa infinitate* verified word for word on p. 78. **Boundary 7
  (p. 84) MEASURED: FORWARDS, split 1/6.** Five things it settles beyond the three conventions above:
  **(a) THE STANDING ORDER PAID A FOURTH TIME** — p. 84 extracted alone and read first; index span
  79–84 right. ⚠ The shape was *overlapping* this time, the safe-looking one, and that predicts nothing:
  `q3-a2` was right on a non-overlapping span and `q2-a2` wrong on one.
  **(b) ⭐ `colcrop.py`'s DEFAULT WAS RIGHT ON ALL SIX LEAVES — the first clean span in this work**
  (79 **1151** · 80 **1360** · 81 **1178** · 82 **1335** · 83 **1198**), ⚠⚠ **and p. 84 still needed the
  REGION treatment**: the five-line block above `ARTICULUS II.` gives **1351/63 px**, Art. II's region
  below it **1358/60 px**, and a window straddling the heading collapses to **5 px**. **A right default
  on five leaves is not a licence to skip the sixth.**
  **(c) ⚠ SIX RAW-VS-PLATE DEFECTS, ALL DIGITS, five of them the 1↔4 and 3↔5 pairs `q1-a1` isolated:**
  `ad i.`→**`ad 4.`** · `pag. 3C0`→**`360`** · `(p. 6.)`→**`(c. 6.)`** · `pag. 13`→**`15`** ·
  `pag. 288`→**`258`** · `a. 4.`→**`a. 1.`**, each settled at 5× or better **against a control in the
  same line**.
  **(d) ⭐ TWO NEAR-MISS MARKS, and the p. 78 one is load-bearing:** a raised mark before *ineffabili*
  (p. 79 arg. 5) and one after *trinitas* in **Quaestio IV's opener** (p. 78, the raw returns
  `trinitas]`). Neither has a seat in its register — p. 78's four entries are all `q3-a2`'s — **so the
  quaestio opener is UNANCHORED, against the working expectation `q2-a1` created.** ⛔ *A mark is not an
  anchor until the register has a seat for it*, and here the register's arithmetic is the whole proof.
  **(e) ⭐ `Aristot., V. Metaph. text 16.` prints with NO point after *text***, against *Metaph.* on the
  same line and `text. 15.`/`text. 12.` on p. 80 — third attestation of the missing-abbreviating-point
  class. ⭐ Also: **p. 80 n. 11 carries an editorial ARGUMENT, not a citation** — the *Respondeo*'s
  opening stretch was already printed at **tom. I, p. 769 n. 7** from **cod. M of Monte Cassino** as an
  additamentum, *ex quo novum arg. insinuatur pro genuitate huius quaestionis*, so the same words stand
  twice in the *Opera omnia* and this project has already published one of them. ⚠ ***quietatur* occurs
  in reply 7 and the *vacatio*/*quies* ruling is LEFT OPEN** — it is a verb; the reserved sites in
  `q5-a2`/`q7-a2` are the noun and the gerund. ⭐ Ten cross-references opened as controls, **all ten
  land**, three of them landing on the *premiss itself* rather than on a gloss (p. 79 n. 10 → `q3-a1`
...
- **✅✅ `bon-qmt-q5-a2` IS BUILT (pp. 93–96, 30 entries, zero `[?]`, 2026-09-07) — QUAESTIO V IS COMPLETE.**
  Eleven arguments that trinity and eternity are incompossible (marginale `Ad oppositum.`), a **`Sᴇᴅ ᴄᴏɴᴛʀᴀ`
  series of ten** (marginale `Fundamenta`, **printed without its final point**), `Cᴏɴᴄʟᴜsɪᴏ` in-column, a
  *Respondeo* in **three declared moves** (*ex parte conditionum essentialium* · *emanationum originalium* ·
  *relationum personalium*, marginalia `Ratio 1.` / `Ratio 2, trimembris.` / `Ratio 3.`), and **eleven replies
  for eleven arguments, none combined**, answering the **OPENING** series — the reverse of `q5-a1` inside one
  quaestio, exactly as Quaestio IV did. Registry untouched. Five things it settles:
  **(a) ⭐⭐⭐ *VACATIO*/*QUIES* RULED, AND A STEM CENSUS COUNTED A HYPHENATED HALF-WORD AS A HIT.** See the
  register note above. ▶ **When a census hit is load-bearing, put it on the plate before believing it** — the
  pilot's near-miss was a pattern that returned too little, this is the same pattern returning too much.
  **(b) ⭐⭐ BOUNDARY 10 (p. 96) MEASURED: it FORWARDS IN THE BODY ONLY, split 5/0** — `QUAESTIO VI.`, its
  subtitle and its **unanchored** opener stand on p. 96 and belong to `q6-a1`; all five register entries anchor
  in Art. II's replies 8, 9, 9, 11, 11. **Same shape as boundary 6 (p. 78), the second body-only forward.**
  ▶ **Ten boundaries, EIGHT shapes: 5/1 · LEAF EDGE · 2/8 · 1/4 · 4/2 · 4/0 · 1/6 · 3/2 · LEAF EDGE · 5/0**
  — 8 forwarding / 2 leaf edges. ⭐⭐ **And the index line `93–96` is right at BOTH ENDS, the first in this work
  verified sound on both sides** — a measurement only because p. 96 was extracted ALONE and read first, the
  standing order paying a **seventh** time. ⛔ **A line being right is a fact about that line, not about the index.**
  **(c) ⭐⭐ TWO FREE CONTROLS, BOTH INTERNAL, BOTH BEATING THE GLYPH:** p. 96 n. 1's `pag. 14` (raw `pag. 1i`)
  → **`bon-qsc-q3` p. 14 n. 5, the *Sophistici Elenchi* locus for *figura dictionis***, the note's own phrase;
  p. 96 n. 5's `pag. 71` → **`bon-qmt-q3-a1` p. 71 n. 2, Augustine *de Trin.* VII.4.7 *Verius cogitatur Deus,
  quam dicitur***, word for word what the body echoes — **a fourth internal cross-reference closing inside the
  work in the fourth of five consecutive chunks.** ⚠ Five raw-vs-plate digit defects, and p. 93 n. 2's own last
  line (`taph. tr. 4. c. 1.`) sets a true `4` and a true `1` **four words apart**, correcting n. 3's `(c. 1.)`
  and n. 8's `c. 4. et 5.` **in opposite directions**.
  **(d) ⭐ RULING 5 IS MET FOR THE THIRD TIME IN THE WORK AND THE RENDERING IS QUOTED:** p. 94 n. 5's
  `fundam. 10.` → *fundamentum 10*, the first such site outside `q3-a2` and `q4-a1`. ⛔ **Ruling 2 exercised on
  ONE SIDE ONLY for the second time**: *lux* **four** times (all in `Sed contra` 5's *lux increata*/*lux creata*),
  *lumen* **ZERO** — all four → "light", nothing transliterated; **record it, or a later reader finds four bare
  "light"s and takes them for a lapse.** *intellect-* → "understanding" with zero bare "intellect";
  *intelligentia* and *contuit-* absent.
  **(e) ⚠⚠ GUTTERS WRONG ON THREE OF FOUR, AND p. 93 CARRIES THE WORK'S MOST HEAVILY INKED CENTRE RULE.**
  93 **1188** (default 1214 on a **4 px** run, 26 px out; rule island peaks at **1242**, beating p. 89's 991) ·
  94 **1338** (the only sound default, 61 px, reproduced) · 95 **1138** (default 1163 on a 3 px run, 25 px out) ·
  96 **1391** for Art. II's REGION — the whole-leaf default gave 1381 on a 3 px run with a **203 px** window
  spread, the leaf stacking three regions. ⭐ **One gutter runover (p. 95 n. 3), ZERO page-crossing — the negative
  established by reading p. 97's register head**, which opens numbered. **Marginalia 19, none clipped** (fifth
  scoping of the 69–73 clipping); the solution-heavy split is total a **seventh** time — twenty-one arguments
  carry **two** glosses, the *Conclusio*, *Respondeo* and eleven replies **seventeen**. ⚠ **p. 95's body raw is
  degraded and was re-set from the plate.** ⚠ **Three `build-citations.py` RESOLVER artefacts are named in the
  chunk's Notes and reserved for the work-close gate** — a bare `pag. N` inherits the *volume*, not the note's own
  last-named tome (p. 93 n. 2, twice), and `ibid.` is carried by **cross-note** anaphora (p. 94 n. 8). **24 records,
  ZERO dangling, ZERO unresolvable, ZERO ambiguous.**

- **✅ `bon-qmt-q6-a1` IS BUILT (pp. 96–102, 44 entries, zero `[?]`, 2026-09-13) — QUAESTIO VI IS OPENED.**
  Registry **division 6** went in with it. Ten arguments, a `Sᴇᴅ ᴄᴏɴᴛʀᴀ:` series of seventeen along a *via
  septiformis*, seventeen replies in eleven paragraphs answering the `Sed contra`. **Boundary 11 (p. 102)
  FORWARDS, split 3/1 — eleven boundaries, NINE shapes** (…5/0 · 3/1), 9 forwarding / 2 leaf edges; n. 4
  forwards to `q6-a2` and runs on to p. 103. Four transferable things:
  **(a) ⭐ A NOTE'S VARIANT SEATS ITS ANCHOR, a second time:** p. 102's raw shows a mark after reply 15's *quod*
  and another after *divina*; n. 2 reads `G addit potentia vel`, which can stand only after *divina* — and the
  *quod* mark sits below the baseline at 5×. ▶ **Read a variant note's lemma before choosing between two marks.**
  **(b) ⭐ WRONG SORTS ARE A CLASS DISTINCT FROM FOULED ONES.** *compa-**rattonis*** (two crossed t's for *ti*,
  7×) and `p. J.` (a real J with a descender for roman *I*, 5×) are letters the forme carried wrongly →
  **transcribed as printed, never emended**, English gives the sense. A letter PRESENT under a stray stroke
  (*stare*, *S.*) is a fouled sort → read plain. ⚠ **A wrong sort inside a citation coordinate breaks the
  resolver** (`p. J.` resolves only to distinctio level) — a resolver matter for the work-close gate, not an edit.
  **(c) ⭐ `vide IV. d. N` after a `II. Sent.` chain is flagged DANGLING by `build-citations.py` though the
  target exists** — a book numeral without `Sent.` is not taken as a new book. The work's second named resolver
  class (after `q5-a2`'s bare `pag. N` tome inheritance); **both for the work-close gate.**
...
- **✅✅ `bon-qmt-q7-a2` IS BUILT (pp. 109–112, 19 entries, zero `[?]`, 2026-09-15) — QUAESTIO VII IS COMPLETE.**
  Nine arguments (`Ad oppositum.`), a `Cᴏɴᴛʀᴀ:` series of six (`Fundamenta.`), `Cᴏɴᴄʟᴜsɪᴏ` in-column, a *Respondeo* on a
  **threefold will** (*accedens* / *antecedens* / *concomitans et acceptans*) set against Art. I's threefold necessity,
  three *rationes* and a `Corollarium`, nine replies answering the **OPENING** series (the reverse of `q7-a1`, as in
  Quaestiones IV–VI). Registry untouched. **Boundary 14 (p. 112) FORWARDS, split 3/2 — fourteen boundaries, TWELVE
  shapes** (…6/1 · 3/2), 12 forwarding / 2 leaf edges; **the index's `109–111` is wrong at its end by a leaf** (the
  `q2-a2` mechanism); the raw band exact a fifth time. Four transferable things:
  **(a) ⭐⭐⭐ THE *vacatio* DECIDING SITE'S OWN NOTE DOUBTS THE WORD** — see the register note above. **Read the
  apparatus of a banked deciding site before treating it as evidence.**
  **(b) ⭐ AN OBJECTION CAN BE MISSING FROM THE BODY AND SUPPLIED ONLY IN A NOTE:** reply 1 answers *de auctoritate
  Augustini et Hilarii*, but only the Augustine stands among the arguments; p. 111 n. 2 supplies the Hilary from
  I *Sent.* d. 6 q. 1 arg. 2 — **in the apparatus, not the body**, which was left as printed. *Nine replies for nine
  arguments* is therefore true of the numbering and not of the authorities.
  **(c) ⭐ A GREEK SORT FOR A LATIN LETTER is an identifiable wrong sort:** p. 111 n. 2 `[η. 58. XXV.]` (η for *n*),
  transcribed as printed (the `q6-a1` `p. J.` rule); `58` fixed by the target `bon-sent-I-d6-a1-q1` n. 5. And a **printed
  dittography** across a line-break, `simi-`|`militer`, transcribed *simimiliter*.
  **(d) Ruling 2 exercised on BOTH sides** (*lucis* → light, *luminis* → lumen); Ruling 5 not reached — **but p. 112 n. 4
  (`q8`'s) carries `fundam. 6.`**. Gutters 109 **1162** (Art. II region) · 110 **1387** (default 1363/16 px) · 111 **1194**
  (default 1161/**2 px**) · 112 **1380** (Art. II region). Q. VIII's opener unanchored by count (five marks, five seats).

- **✅✅✅ `bon-qmt-q8` IS BUILT (pp. 112–115, 22 entries, zero `[?]`, 2026-09-15) — THE WORK'S LAST CHUNK; THE *DE MYSTERIO
  TRINITATIS* IS COMPLETE.** Registry **division 8** went in with it (*Utrum possit simul stare trinitas cum summa primitate*,
  read on p. 112). Seven arguments (`Ad oppositum.`), a `Sᴇᴅ ᴄᴏɴᴛʀᴀ:` series of eight (`Fundamenta.`), `Cᴏɴᴄʟᴜsɪᴏ` in-column,
  a *Respondeo* on supreme actuality / fontality / fecundity and a `Corollaria.` gathering Questions II–VII, seven replies in
  six paragraphs answering the **OPENING** series. **The work ends on p. 115 with an ornamental rule and NO colophon**, fixed
  positively from p. 117's `DE PERFECTIONE EVANGELICA` heading (p. 116 blank, read on the plate). Four transferable things:
  **(a) ⭐ Q. VIII's opener is UNANCHORED on the plate, as `q7-a2` predicted by count — eight openers, two anchored (pp. 59, 106).**
  **(b) ⭐⭐ Ruling 5 MET TWICE and both quoted** (p. 112 n. 4 *fundamentum 6*; p. 113 n. 10 *fundamentum 12 ff.*) — seven sites
  in the work. **(c) ⭐ p. 117's proemium *Volentes¹* IS ANCHORED** — the work-4 prediction answered, the opposite of p. 45.
  **(d) ⚠ A sixth anaphora artefact for the gate:** p. 114 n. 3's *contra Faustum, loc. cit.* resolves by cross-note anaphora to
  I *Sent.* d. 19. Gutters 112 **1387** (Q. VIII region) · 113 **1156** (default 1127/3 px) · 114 **1403** · 115 **1185**
  (default 1156/9 px). Wrong sort *jnaequale*, `Joan.`, Quaracchi's *Manifestum omissionem* — all as printed.
...
- **✅ `bon-qmt-q1-a2` IS BUILT (pp. 51–58, 53 entries, zero `[?]`, 2026-09-05) — QUAESTIO I IS COMPLETE.**
  ...
  `SED CONTRA` series** in two runs, an **incidental question** on the motive of faith, `CONCLUSIO`,
  a *Respondeo* on the three books (*liber creaturae / Scripturae / vitae*), and **twelve replies**.
  Registry untouched — division 1 was already in. **Boundary 2 (p. 58) MEASURED: it does NOT
  forward** (Art. II closes two-fifths down the right column, rest of the leaf white, no part of
  Quaestio II on it). Five things it settles:
  **(a) ⛔⛔ THE REPLIES ARE PRINTED OUT OF ORDER — 10, 12, 11 — AND QUARACCHI SAYS SO.** p. 58 n. 2
  ends `— Nota inversionem ordinis argg.` Transcribed as printed. ⭐ **The lesson is the direction of
  the catch:** a gap in a numbered series (…10, 12…) reads as a *dropped unit* — the d.27 failure
  mode — and the reflex is to hunt for missing text; here the unit was on the next column, out of
  order, and the register two inches below named the anomaly. **Read the page's register before
  concluding a numbered series has lost a member.**
  **(b) ⚠⚠ `colcrop.py`'s DEFAULT ROW WINDOW FAILED ON p. 55 IN A NEW WAY, AND p. 58 SHOWS THE
  MIRROR.** p. 55: the script's own body window (rows 0.45–0.92) gave **1227 on a 3 px run**;
  rows **0.20–0.50** gave **1199 on a 62 px run**, stable across three overlapping windows — 1199
  adopted, and the crops confirm it. The leaf's *lower* half is what closes the gutter up. p. 58:
  body windows all agree at **1294–1297** (adopted 1296), but rows 0.60–0.90 return **1389 on a
  247 px run** — that is the **white space below the article's close**, not a gutter, and a wider
  window would have taken it. **Two leaves, two opposite reasons the default window is the wrong
  window. Profile; do not just re-run the default and read its number.**
  **(c) ⭐⭐ A QUARACCHI BACK-REFERENCE IS A FREE, DECISIVE CONTROL ON ITS OWN PAGE NUMBER.**
  p. 54 n. 4 (*Gregor. supra pag. 53, nota 3*) and p. 56 n. 4 (*cfr. supra pag. 52, nota 7*) were
  both OCR'd with a `3` for the `5`. Neither was settled by the glyph: **p. 53 n. 3 IS the Gregory
  homily and p. 52 n. 7 IS the II Cor. 10, 5 note**, both inside this same chunk. Open the named
  note before straining at the type. Six raw-vs-plate digit defects in all (`p. X. c. S.`→**c. 5.** ·
  `pag. 33`→**53** · `d. II. c. i.`→**c. 4.** · `Psalm. i, 1`→**Psalm. 4, 7** · `pag. 32`→**52** ·
  `q. L ad 2`→**q. 4. ad 2.**). ⚠ The 1-vs-4 calibration was **re-run, not inherited**, at 3.0–3.2×
  against same-line controls, and held every time — including on the inherited p. 51 n. 6, where
  `Vers. 19` and `Marc. 16, 16` both read as `4`s at 1.6×.
  **(d) ⚠ THE OPPOSING-SERIES HEADING IS NOW THREE SHAPES IN VOL V, AND TWO CONSECUTIVE CHUNKS OF
  THIS WORK DISAGREE.** `bon-qsc-q1`/`q2` print `Cᴏɴᴛʀᴀ:` · `bon-qsc-q3` prints `Sᴇᴅ ᴄᴏɴᴛʀᴀ:` ·
  `bon-qmt-q1-a1` prints **no heading at all** (running italic formula) · `bon-qmt-q1-a2` prints
  `Sᴇᴅ ᴄᴏɴᴛʀᴀ:` in letter-spaced small caps, in-column. **The find-it-by-content rule is not
  retired.** ⭐ And the reply series again answers the OPPOSING side (the *Respondeo* concedes the
  affirmative), **twelve replies for thirteen arguments**: 5 and 6 are printed combined, and
  argument 13 is answered by the incidental question, not by a reply — Quaracchi's own marginale
  says so (`Ad arg. 13. circa quaestionem incid.`). **Never assume the reply count matches.**
  **(e) ⚠ TWO PRINTING DEFECTS RECORDED AS PRINTED:** p. 57 n. 7 prints **`Cfc. Ioan. 3, 16.`** —
  `Cfc.`, not `Cfr.`, with plate and raw agreeing (two independent witnesses); and **p. 58 n. 6
  carries a blank gap of about a dozen characters inside its scripture quotation**, where the
  Vulgate has *Christi et*. Both reproduced, neither completed.
  ⭐ Register: **ruling 2 does its heaviest work in the work so far** — *lumen inditum/infusum*,
  *lumen divini vultus*, *lumen aeternum* against *lux vera*, *lux hominum*, *influentia lucis*,
  *lux desuper infusa*, and **the pair stands in one sentence** (*liber iste vitae est lux vera…
  Illuminat autem dupliciter, scilicet per lumen inditum et per lumen infusum*). ⚠⚠ **The Psalm 4:7
  site RECURS at p. 55 n. 9** — same verse as p. 50 n. 2, and here it is the **anchor for the body's
  own *lumen divini vultus***, so note and body must agree; ruling 2 was **followed, not suspended**,
  and the site is handed to the shakedown gate **now attested twice and load-bearing the second
  time**. **Do not settle it in a chunk.** *intellectus* → "understanding" at every site, zero bare
  "intellect". Ruling 5 not reached; *vacatio*/*quies* not testable here.
  ⭐ Gutters: 51 **1182** (carried) · 52 **1396** · 53 **1140** · 54 **1382** · 55 **1199** ·
  56 **1389** · 57 **1183** · 58 **1296**. One gutter runover (p. 53 n. 4), none page-crossing.
  ⚠ On p. 57 the `S. Bonav. — Tom. V.` signature line and the gathering signature `8` are not
...
- **⭐⭐⭐ THE WORK-CLOSE REGISTER RULINGS — WILSON, 2026-09-17. All seven ruled in one sitting off a single evidence sheet; SIX HELD WHAT THE CHUNKS BUILT, ONE CHANGED THE TEXT. These are now the work's settled register; do not re-open them.**
  **(1) THE PURITY TRIPLE STANDS AS BUILT: *pudicitia* → purity · *puritas* → pureness · *munditia* → cleanness** (*castitas* → chastity, separately attested in all three `q3` chunks). Decided on `q3-a3` p. 179 reply 8, where *munditia*, *pudicitia coniugalis* and *puritatis et munditiae* stand in ONE sentence: three Latin words in one clause force three English words, and the natural-English swap (*puritas* → purity, *pudicitia* → chastity) collides with *castitas*. "Pureness" is the price of the parallel and Wilson paid it knowingly. ▶ ***purus* (I Tim. 1:5) → "pure heart"** in Douay's shape — the ADJECTIVE does not join the triple. 0 lines changed.
  **(2) ⭐ *dominium* IS SPLIT BY SENSE — THE ONLY RULING THAT CHANGED TEXT: "ownership" for the property sense (`q2-a1` p. 131 *usum… non dominium*), "LORDSHIP" for political rule (`q4-a1` p. 181 *dominium potentiae seu praesidentiae* → "lordship of power or presidency").** ▶ **The argument that decided it was the CENSUS, not the sense: two sites in 82 printed pages, one per sense**, so the one-word rule was being kept for a word the work barely uses. A split that costs 2 Latin + 2 English lines is cheaper than a strained rendering standing in a *Respondeo*. ⛔ **The general lesson: before defending a one-word rule, COUNT THE SITES — the cost of a split is a function of n, and at n=2 the rule is protecting nothing.** Applied 2026-09-17 in `q4-a1` (body + status line + Notes bullet).
  **(3) *ius*/*lex* → "right"/"law" HELD, INCLUDING *ius canonicum* → "canon right".** 83 *ius*-forms; only the eight *ius canonicum* read unidiomatically, and `q4-a3` p. 195's Epilogus chains all four members (*legi naturae* · *legi scriptae* · *iuri pontificio* · *iuri canonico*). **The carve-out was refused because it would flatten the one sentence the distinction was built for** — contrast (2), where no such sentence existed. 0 lines changed.
  **(4) "CONJUGAL CONTINENCE" LEFT AS PRINTED, NO NOTE:** `q3-a1`'s title follows *pudicitia*, its ~14 argument sites follow *continentia*, and **reply 15 reconciles them in Bonaventure's own voice** (*quae magis proprie coniugalis pudicitia dicitur*). ▶ **Where the text explains its own asymmetry, a translator's note is redundant** — the `tr-` mechanism is for what the Latin holds together, not for what the author already unpacks.
  **(5) *Antistes* → "Prelate" (capitalised) KEPT** at its one site (`q4-a3` p. 194 Conclusio, *summum et primum Antistitem*); *Pontifex*/*sacerdos*/*episcopus*/*hierarcha*/*primates* are all taken, and the capital carries the distinction against the chunk's eight lower-case *praelatus* sites.
  **(6) *necessitas tentionis* → "necessity of holding" and *ordo dignativus* → "the order of deigning" KEPT, no notes** (`q4-a2` p. 186). ▶ **The strain is Bonaventure's coinage, not a translation artefact, and a literal rendering is the honest one**; "condescension" was rejected as importing a word he did not use and a modern pejorative.
  **(7) THE pp. 155–156 ADDITAMENTUM IS NOT TRANSCRIBED — LEFT AS `q2-a2` p. 155 n. 8 DESCRIBES IT.** ▶ **The distinguishing fact against the `bon-hex-c23` precedent is the LEAF, not the content**: hex-c23's additamentum closed on the leaf it opened on, this one straddles the Art. II / Art. III boundary and belongs to neither chunk. ▶ **What licensed leaving it: n. 8 already prints, in both languages on the live site, the addition's existence, its extent (nine authorities), its contents and the editors' *non genuinum* reasoning** — the reader loses the nine texts, not the knowledge that they exist. ⛔ Not a precedent for suppressing an editorial appendix that has NO such note.
...
- **✅ WORK-CLOSE GATE RUN AND CLOSED 2026-09-19 — ZERO TEXT DEFECTS; VOLUME V IS CLOSED** (`manual-review/vol5-sermones-selecti-workclose-gate.md`). Five chunks, pp. 535–579, **456 entries**; boundaries 4/4 digit for digit, every page owned once, registers contiguous; all seven rulings verified as applied (*magister* re-derived over the live region: 45 → teacher 38 / teaching office 4 / master 0). Repairs were documentation only: `s3`/`s4`'s forwarded "five of five on the Explicit" withdrawn (**four of five**), `s3`'s *lumen* count 5 → 7. ⚠ **Ruling 6's "one site" was a LOW count** — *puritas* has two noun sites + *impuritas*, all p. 546, all "purity"; ruling unaffected. ⚠ **The second named-work reference to the Tractatus is in `bon-hex-scholion`, not `bon-don-c1`.** Pass 4 reclaimed ~621 MB; **p. 535's mirror-flipped plate is now deleted — any re-extract brings the mirror back.** ▶ **A sentence about the WHOLE work in a chunk's Notes is a claim for the gate to re-derive, never a finding** — four such sentences said "five of five" and one "closed the question so the gate would not re-open it." Committed locally; NOT pushed, NOT deployed.
